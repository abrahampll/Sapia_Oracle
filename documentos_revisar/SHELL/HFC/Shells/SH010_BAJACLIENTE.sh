...
EOP
	...
	if [ "$FINDFILE" = "" ] && [ "$FINDBKP" = "" ] ; then
		...
		pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto" $'\n' "de El Archivo de Puntos por Baja no se encuentra en la ruta" 
		echo "Buen día, no se encontraron los siguientes archivos $ARCHNAME en la carpeta de $DIRPROCHFC_PROBAJ." $'\n' "Favor de atender este inconveniente" $'\n' "Gracias."$'\n'  | mail -s "HFC-POSTPAGO: El Archivo de Baja Puntos no se encuentra en la ruta $DIR_PROC_PAGOS." $IT_MAIL
		FinalShell
		exit
	fi
	...
	pMessage "Ejecución de proceso de BAJA DE HFC fue satisfactorio"
