#! /bin/ksh
#*************************************************************
#* DESCRIPCION           : Puntos por Promocion DTH
#* EJECUCION             : Control-D                            
#* AUTOR                 :                                   
...
CONTROL=$DIRCONTROLDTH/importaPPromDTH.ctl
BAD=$DIRFALLOSDTH/importaDTH_BAD_$FECHA.bad
FILELOG=$DIRLOGDTH/SH004_PROC_ARC_PROMO_$FECHA.log
CTL_LOG=$DIRLOGDTH/CTL004_LOG_$FECHA.log
...
pMessage "*************************************" 
pMessage "Iniciando subproceso de Importación              " 
...
if [ "$FILEDATA" = "" ] ; then
   ...
   pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de El Archivo de Promocion DTH no se encuentra en la ruta" 
   echo "Buen día, no se encontraron los siguientes archivos $ARCHNAME en la carpeta de $RUTANAME." $'\n' "Favor de atender este inconveniente" $'\n' "Gracias."$'\n'  | mail -s "El Archivo de Promocion DTH no se encuentra en la ruta." $IT_MAIL 
...
if [ $CANT_DATA = 0 ] ; then
	...
	pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de El Archivo de Promocion DTH se encuentra vacio." 
	echo "Buen día, los siguientes archivos $ARCHNAME en la carpeta de ORIGEN no contienen datos." $'\n' "Favor de atender este inconveniente" $'\n' " Gracias." $'\n'  | mail -s "El Archivo de Promocion DTH se encuentra vacio" $IT_MAIL
...
if [ "$promo" == "" ] || [ "$user" == "" ] ; then
   ...
   pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de El Archivo de Promocion DTH no se encuentra en forma Correcta" 
   pMessage "Termino subproceso"
   pMessage "************************************" 	    
   pMessage "************************************"
   cat $FILELOG >> $FILELOGPR
   ...
   ARCHIVO=`echo $ARCHNAME | awk 'BEGIN{FS="Promocion/"} {printf $2 }'`
   TIPOCLIE="6"

	while read FIELD03
	do
		if  [ ${contador} != ${numregistros} ] ; then
			echo "${FIELD03}|${PERIODO}|${promo}|${ARCHIVO}|${TIPOCLIE}|${FECHAARCH}" >> $DIRLOGDTH/$TEMP_FILE
			contador=`expr $contador + 1`
		fi

	done < $FILEDATA
	
	FILEDATATEMP=`find $DIRLOGDTH/$TEMP_FILE`
	CANT_DATATEMP=`cat  $DIRLOGDTH/$TEMP_FILE | wc -l | sed 's/ //g'`
	...
	if [ "$FILEDATATEMP" = "" ] || [ $CANT_DATATEMP = 0 ] ; then
	   ...
	   pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de El Archivo de Promocion DTH no contiene datos validos" 
	   echo "Buen día, el archivo $NomArchivo en la carpeta $RUTANAME no contiene datos validos a procesar." $'\n' "Favor de atender este inconveniente" $'\n' "Gracias."$'\n'  | mail -s "DTH - POSTPAGO: El Archivo de Promocion DTH no contiene datos validos." $IT_MAIL 
	   #pMessage "Ruta del Archivo log : " $FILELOG
	   pMessage "Termino subproceso"
	   pMessage "************************************" 	    
	   pMessage "************************************" 
	...
	pMessage "Se procede a importar los datos del archivo de entrada a la tabla de promocion DTH"
	...
	if [ $VALIDAT_CTL -ne 0 ] ;
		...
		echo "Buen día, ocurrio un error al momento de importar los datos del archivo $DIRFALLOSDTH/$NomArchivo  a la tabla de promocion DTH." $'\n' "Favor de atender este inconveniente" $'\n' " Gracias." $'\n'  | mail -s "Error al importar datos" $IT_MAIL
		cat $CTL_LOG >> $FILELOG
		rm -f $CTL_LOG 
		pMessage "Termino subproceso"
	    pMessage "************************************" 	    
	...
	pMessage "Se procede a ejecutar el SP PKG_CC_PTOSFIJA.ADMPSI_PROMDTH_DTH"
...
BEGIN
	...
	begin
		...
		$PCLUB_OW.PKG_CC_PTOSFIJA.ADMPSI_PROMDTH_HFC('6',v_usuario,v_fecha,'$NomArchivo',
...
EOP
	...
	if [ ${VALIDA_EJEC_SP} -ne 0 ] || [ ${VALIDA_EJEC_SP_B} -ne 0 ] ; then
		pMessage "Hora y Fecha: $demora"
		pMessage "Hubo un error durante la ejeción del SP PKG_CC_PTOSFIJA.ADMPSI_PROMDTH_DTH" 
		pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de IMPORTACION DE PUNTOS POR PROMOCION DTH – Se encontraron errores" 
		echo "Buen día, ocurrio un problema al ejecutar el SP de PKG_CC_PTOSFIJA.ADMPSI_PROMDTH_DTH." $' \n' "Favor de atender este inconveniente" $'\n' "Gracias." $'\n'  | mail -s "IMPORTACION DE PUNTOS POR PROMOCION DTH – Se encontraron errores" $IT_MAIL 		
		pMessage "Se envió correo Fecha y Hora: $demora" 		
		pMessage "Termino subproceso"
	    pMessage "************************************" 	    
		pMessage "************************************" 
	...
	FINDBKP=`find $DIR_PROC_PROM/$NomArchivo`
	...
	if [ "$FINDFILE" = "" ] && [ "$FINDBKP" = "" ] ; then
		...
		echo "Buen día, no se encontraron los siguientes archivos $ARCHNAME en la carpeta de $RUTANAME." $'\n' "Favor de atender este inconveniente" $'\n' "Gracias."$'\n'  | mail -s "DTH - POSTPAGO: PROMOCION, no se encuentra el archivo." $IT_MAIL 
		pMessage "Se envió correo Fecha y Hora: $demora"		
		pMessage "Termino subproceso"
	    pMessage "************************************" 	    
		pMessage "************************************"
...
pMessage "Termino subproceso Asignacion de Puntos por Promocion DTH ClaroClub"
pMessage "********** FINALIZANDO SUBPROCESO ********** " 
pMessage "Fin de subproceso "
pMessage "************************************" 
#pMessage "Ruta del Archivo log : ${FILELOG}" 
