create or replace package body PKG_CLAROCLUB is
...
C_APE_MAT         ADMPT_TMP_ACCAMO_IB.ADMPV_APE_MAT%type;
...
-- DATOS DWH
V_NRO_DOC_DWH       VARCHAR(20);
V_TIPO_DOC_DWH      VARCHAR(30);
V_COD_TD_DWH        CHAR(1);
V_EXIST             NUMBER;
...
V_FIRST_CALLDATE    DATE;
V_NRO_DOC_DWH2      VARCHAR(20);
V_TIPO_DOC_DWH2     VARCHAR(30);
V_COD_TD_DWH2       CHAR(1);
V_FIRST_NAME2       VARCHAR(200);
V_LAST_NAME2        VARCHAR(200);
V_SEXO2             VARCHAR(1);
V_DEPARTAMENTO2     VARCHAR(30);
V_FIRST_CALLDATE2   DATE;

V_TIPODOC_CC        VARCHAR(1);
V_NRODOC_CC         VARCHAR(20);
VL_NUMLINEA         VARCHAR(20);
V_CONT_TIPO         NUMBER;
...
CURSOR CUR_LINEASXCLI(tipo_doc VARCHAR2, num_doc VARCHAR2)is
     ...
     where UPPER(d.X_DOC_TYPE) = tipo_doc
                  ...
                  IF TRIM(C_NUM_LINEA) IS NULL THEN
                     BEGIN
                        ...
                        -- Registra Cliente IB
                          ...
                          FROM pclub.admpt_clienteib a
                          WHERE a.admpv_tipo_doc = V_TIPO_DOC_CLARO
                          and a.admpv_num_doc = C_NUM_DOC
                        ...
                        VALUES ( '1', C_TIPO_DOC, C_NUM_DOC ,C_NUM_LINEA, K_FECHA, NVL(C_NOM_ARCH,''));
                          ...
                          ELSE
                             ...
                             set    admpc_cod_error='24',
                                    admpv_msje_error = 'ACTIVACIÓN YA INFORMADA ANTERIORMENTE'
                             where  admpc_cod_trans= '1' and
                                    admpv_tipo_doc=C_TIPO_DOC and
                                    admpv_num_doc=C_NUM_DOC and admpd_fec_oper=K_FECHA;
                          END IF;
                     ...
                     --LOOP
                       ...
                       IF V_TIPO_DOC_EQUIV=C_CUR_TIP_DOC AND C_NUM_DOC=C_CUR_NUM_DOC THEN
                           BEGIN
                             ...
                             END IF;
                           END;
                         ELSE --El teléfono no pertenece al cliente - deysi
                         ...
                         --CLIENTE PREPAGO
                             ...
                             BEGIN
                             ------------VALIDAR SI ES QUE YA ESTE REGISTRADO

                             SELECT COUNT(*) INTO V_CONT_1 FROM  PCLUB.ADMPT_CLIENTEIB VIB
                             WHERE VIB.ADMPV_TIPO_DOC = V_TIPO_DOC_CLARO AND VIB.ADMPV_NUM_DOC = C_NUM_DOC AND VIB.ADMPC_ESTADO = 'A';
                             ...
                             IF V_CONT_1 > 0 THEN
                                  ...
                                  SET    ADMPC_COD_ERROR='24',
                                         ADMPV_MSJE_ERROR = 'ACTIVACIÓN YA INFORMADA ANTERIORMENTE'
                                  WHERE  ADMPC_COD_TRANS= '1'
                                  AND    ADMPV_TIPO_DOC=C_TIPO_DOC
                                  AND    ADMPV_NUM_DOC=C_NUM_DOC AND admpd_fec_oper=K_FECHA;
                            ...
                            ELSE
                             ...
                             --SI EL CLIENTE PREPAGO ESTA EN CLARO CLUB
                             SELECT NVL(COUNT(*),0) INTO V_COUNT_PRE
                             FROM ADMPT_CLIENTE C
                             WHERE C.ADMPV_COD_CLI=C_NUM_LINEA
                             AND C.ADMPV_COD_TPOCL='3'
                             AND C.ADMPC_ESTADO='A';

                             IF V_COUNT_PRE = 0 THEN
                                BEGIN
                                 --VALIDANDO QUE EL NUMERO EXISTA EN LA TABLA DWH
                                -- Parametros
                                SELECT nvl(PCLUB.admpt_cli_ib_sq.nextval,-1) INTO V_CODCLIIB FROM dual;

                                  SELECT NVL(COUNT(*),0) INTO V_EXIST
                                  FROM dm.BASE_ABONADOS_CLARO@dbl_reptdm_d
                                  WHERE TIPO_ABONADO=1 AND ESTADO IN ( 'ACTIVE','ACTIVO','GRACE')
                                  AND MSISDN  = CONCAT('51',C_NUM_LINEA);

                                  IF V_EXIST <> 0 THEN
                                     -- SI LA LINEA PERTENECE AL CLIENTEIB
                                SAVEPOINT BQ6;
                                      ------------------------------
                                      SELECT X_DOC_NUM,X_DOC_TYPE,S_FIRST_NAME,S_LAST_NAME,X_SEXO,DEPARTAMENTO,FIRSTCALLDATE
                                      INTO V_NRO_DOC_DWH, V_TIPO_DOC_DWH,V_FIRST_NAME,V_LAST_NAME,V_SEXO,V_DEPARTAMENTO,V_FIRST_CALLDATE
                                      FROM dm.BASE_ABONADOS_CLARO@dbl_reptdm_d
                                      WHERE TIPO_ABONADO=1 AND ESTADO IN ( 'ACTIVE','ACTIVO','GRACE')
                                      AND MSISDN  = CONCAT('51',C_NUM_LINEA);

                                      SELECT COUNT(*) INTO V_CONT_TIPO
                                      FROM ADMPT_TIPO_DOC
                                      WHERE UPPER(admpv_equ_dwh) = UPPER(V_TIPO_DOC_DWH);

                                      IF V_CONT_TIPO > 0 THEN
                                      SELECT ADMPV_COD_TPDOC INTO V_COD_TD_DWH
                                      FROM ADMPT_TIPO_DOC
                                        WHERE UPPER(admpv_equ_dwh) = UPPER(V_TIPO_DOC_DWH);
                                      ELSE
                                        V_COD_TD_DWH := '';
                                      ...
                                      IF TRIM(V_TIPO_DOC_CLARO) = TRIM(V_COD_TD_DWH) AND TRIM(C_NUM_DOC) = TRIM(V_NRO_DOC_DWH) THEN
                                        --REGISTRO CLIENTEIB
                                        INSERT INTO PCLUB.admpt_clienteib
                                        (admpn_cod_cli_ib, admpv_tipo_doc, admpv_num_doc, admpv_num_linea, admpv_cod_cli, admpd_fec_act, admpc_estado, admpn_flg_debi, admpn_flg_num, admpn_bono_act, admpv_nom_cli, admpv_ape_cli)
                                        VALUES
                                         (V_CODCLIIB, V_TIPO_DOC_CLARO, C_NUM_DOC, C_NUM_LINEA,C_NUM_LINEA, C_FEC_ACT , 'A', 0, 0, 0, C_NOM_PRI||' '||C_NOM_SEG,C_APE_PAT||' '||C_APE_MAT );
                                        COMMIT;

                                        --REGISTRO CLIENTECC
                                        --obtengo las lineas asociadas al cliente
                                        FOR R IN CUR_LINEASXCLI(V_TIPO_DOC_DWH,C_NUM_DOC) LOOP

                                              SELECT X_DOC_NUM,X_DOC_TYPE,S_FIRST_NAME,S_LAST_NAME,X_SEXO,DEPARTAMENTO,FIRSTCALLDATE
                                              INTO V_NRO_DOC_DWH2, V_TIPO_DOC_DWH2,V_FIRST_NAME2,V_LAST_NAME2,V_SEXO2,V_DEPARTAMENTO2,V_FIRST_CALLDATE2
                                              FROM dm.BASE_ABONADOS_CLARO@dbl_reptdm_d
                                              WHERE TIPO_ABONADO=1 AND ESTADO IN ( 'ACTIVE','ACTIVO','GRACE')
                                              AND MSISDN  = R.MSISDN;
                                              ...
                                              IF V_CONT_TIPO > 0 THEN
                                              SELECT ADMPV_COD_TPDOC INTO V_COD_TD_DWH2
                                              FROM ADMPT_TIPO_DOC
                                                WHERE UPPER(admpv_equ_dwh) = UPPER(V_TIPO_DOC_DWH2);
                                              ELSE
                                                V_COD_TD_DWH2 := '';
                                              END IF;

                                              VL_NUMLINEA:= substr(R.MSISDN, 3,length(R.MSISDN));

                                        SELECT COUNT(*) INTO V_EXIST
                                        FROM ADMPT_CLIENTE
                                              WHERE ADMPV_COD_CLI = VL_NUMLINEA;

                                        IF V_EXIST = 0 THEN
                                          INSERT INTO ADMPT_CLIENTE
                                          (ADMPV_COD_CLI,ADMPV_COD_SEGCLI,ADMPN_COD_CATCLI,ADMPV_TIPO_DOC,ADMPV_NUM_DOC,ADMPV_NOM_CLI,
                                          ADMPV_APE_CLI,ADMPC_SEXO,ADMPV_EST_CIVIL,ADMPV_EMAIL,ADMPV_PROV,ADMPV_DEPA,ADMPV_DIST,
                                          ADMPD_FEC_ACTIV,ADMPV_CICL_FACT,ADMPC_ESTADO,ADMPV_COD_TPOCL)
                                          VALUES
                                                (VL_NUMLINEA,'','2',V_COD_TD_DWH2,V_NRO_DOC_DWH2,V_FIRST_NAME2,
                                                 V_LAST_NAME2,V_SEXO2,'','','',V_DEPARTAMENTO2,'',
                                         V_FIRST_CALLDATE2,'','A','3');
                                          COMMIT;
                                        END IF;
                                              IF VL_NUMLINEA=C_NUM_LINEA THEN
                                        ---REGISTRO SALDO
                                        SELECT COUNT(*) INTO V_EXIST
                                        FROM admpt_saldos_cliente
                                        WHERE admpn_cod_cli_ib = V_CODCLIIB;

                                        IF V_EXIST = 0 THEN
                                          INSERT INTO PCLUB.admpt_saldos_cliente (admpn_id_saldo, admpv_cod_cli,admpn_cod_cli_ib,admpn_saldo_cc,admpn_saldo_ib,admpc_estpto_cc,admpc_estpto_ib)
                                          VALUES (PCLUB.admpt_sld_cl_sq.nextval, C_NUM_LINEA, V_CODCLIIB, 0, 0,'A', 'A' );
                                          COMMIT;
                                        END IF;
                                      ELSE
                                                  INSERT INTO PCLUB.admpt_saldos_cliente (admpn_id_saldo, admpv_cod_cli,admpn_cod_cli_ib,admpn_saldo_cc,admpn_saldo_ib,admpc_estpto_cc,admpc_estpto_ib)
                                                  VALUES (PCLUB.admpt_sld_cl_sq.nextval, VL_NUMLINEA, '', 0, 0,'A','');
                                                  COMMIT;
                                              END IF;
                                ...
                                COMMIT;
                                  ...
                                  ELSE
                                      ...
                                      -- REGISTRO EN LA TABLA AUX
                                      INSERT INTO PCLUB.admpt_aux_accamo_ib (admpc_cod_trans, admpv_tipo_doc, admpv_num_doc, admpv_num_linea, admpd_fec_oper, admpv_nom_arch)
                                      VALUES ( 1, C_TIPO_DOC, C_NUM_DOC ,'', K_FECHA, C_NOM_ARCH);
                                      COMMIT;
                             ...
                             ELSE
                               BEGIN
                                 ...
                                 SELECT C.ADMPV_TIPO_DOC,C.ADMPV_NUM_DOC INTO V_TIPODOC_CC,V_NRODOC_CC
                                 FROM ADMPT_CLIENTE C
                                 WHERE C.ADMPV_COD_CLI=C_NUM_LINEA
                                 AND C.ADMPV_COD_TPOCL='3'
                                 AND C.ADMPC_ESTADO='A';
                                 ...
                                 IF TRIM(V_TIPO_DOC_CLARO) = TRIM(V_TIPODOC_CC) AND TRIM(C_NUM_DOC) = TRIM(V_NRODOC_CC) THEN
                                    --INSERTO CLIENTEIB
                                    INSERT INTO PCLUB.admpt_clienteib
                                    (admpn_cod_cli_ib, admpv_tipo_doc, admpv_num_doc, admpv_num_linea, admpv_cod_cli, admpd_fec_act, admpc_estado, admpn_flg_debi, admpn_flg_num, admpn_bono_act, admpv_nom_cli, admpv_ape_cli)
                                    VALUES
                                    ...
                                    --INSERTO SALDO
                                    SELECT COUNT(*) INTO V_EXIST
                                    FROM PCLUB.admpt_saldos_cliente A
                                    WHERE A.ADMPV_COD_CLI = C_NUM_LINEA;

                                    IF V_EXIST > 0 THEN
                                      UPDATE PCLUB.admpt_saldos_cliente
                                      SET ADMPN_COD_CLI_IB = V_CODCLIIB,
                                          ADMPC_ESTPTO_CC = 'A',
                                          ADMPC_ESTPTO_IB = 'A'
...
procedure ADMPSI_ENT_BON_PRE (K_CODERROR OUT NUMBER, K_DESCERROR OUT VARCHAR2, K_NUMREGTOT OUT NUMBER, K_NUMREGPRO OUT NUMBER, K_NUMREGERR OUT NUMBER)
IS
--****************************************************************
-- Nombre SP           :  ADMPSI_ENT_BON_PRE
-- Propósito           :  Actualizar los puntos ganados por el cliente IB asociado a una línea prepago desde el primer envió de información realizado por Interbank. Esto para que pueda canjear sus puntos IB y CC.
...
SELECT  admpc_cod_trans,
        ...
        admpc_cod_error,
        admpv_msje_error
FROM PCLUB.admpt_imp_accamo_ib WHERE TRIM(ADMPC_COD_TRANS) ='1' and admpv_num_linea<>'000000000' AND admpc_cod_error='22' ;


-- CAMPOS DE LA TABLA IMP_ACCAMO_IB
C_COD_TRANS       ADMPT_IMP_ACCAMO_IB.admpc_cod_trans%type;
C_TIPO_DOC        ADMPT_IMP_ACCAMO_IB.admpv_tipo_doc%type;
C_NUM_DOC         ADMPT_IMP_ACCAMO_IB.admpv_num_doc%type;
C_NOM_PRI         ADMPT_IMP_ACCAMO_IB.admpv_nom_pri%type;
C_NOM_SEG         ADMPT_IMP_ACCAMO_IB.Admpv_Nom_Seg%type;
C_APE_PAT         ADMPT_IMP_ACCAMO_IB.admpv_ape_pat%type;
C_APE_MAT         ADMPT_IMP_ACCAMO_IB.admpv_ape_mat%type;
C_NUM_LINEA       ADMPT_IMP_ACCAMO_IB.admpv_num_linea%type;
C_FEC_ACT         ADMPT_IMP_ACCAMO_IB.admpd_fec_act%type;
C_ACEP_BONO       ADMPT_IMP_ACCAMO_IB.admpc_acep_bono%type;
C_FEC_OPER        ADMPT_IMP_ACCAMO_IB.admpd_fec_oper%type;
C_NOM_ARCH        ADMPT_IMP_ACCAMO_IB.admpv_nom_arch%type;
C_COD_ERROR       ADMPT_IMP_ACCAMO_IB.admpc_cod_error%type;
C_MSJE_ERROR      ADMPT_IMP_ACCAMO_IB.admpv_msje_error%type;
...
-- PREPAGO
V_COUNT_PRE NUMBER;
V_CONTPRE_SALDO NUMBER;
V_PRE_PUNTOS NUMBER(4);
...
V_DEPARTAMENTO2     VARCHAR(30);
V_FIRST_CALLDATE2   DATE;
V_ESTADO2           VARCHAR(30);

V_TIPODOC_CC        VARCHAR(1);
V_NRODOC_CC         VARCHAR(20);
V_FECHA             DATE;
V_SALDO_IB          VARCHAR(20);
VL_NUMLINEA         VARCHAR(20);
V_CONT_TIPO         NUMBER;



CURSOR C_LINEASXCLI (tipo_doc VARCHAR2, num_doc VARCHAR2)is
        select d.MSISDN
          from dm.BASE_ABONADOS_CLARO@dbl_reptdm_d d
          where UPPER(d.X_DOC_TYPE) = (SELECT UPPER(A.ADMPV_EQU_DWH) FROM ADMPT_TIPO_DOC A WHERE A.ADMPV_COD_TPDOC = tipo_doc)
           and d.X_DOC_NUM=num_doc
           AND TIPO_ABONADO = 1
           AND ESTADO IN ('ACTIVE','ACTIVO','GRACE');
BEGIN

    V_FECHA := TO_DATE(TRUNC(SYSDATE));
    K_CODERROR:=0;
    K_DESCERROR:='';
    ...
    --Validaciones para Prepago
    SELECT nvl(admpv_cod_tpdoc,'-1') INTO V_TIPO_DOC_CLARO FROM PCLUB.ADMPT_TIPO_DOC WHERE admpv_cod_equiv=C_TIPO_DOC;
    SELECT nvl(admpv_dsc_docum,'-1') INTO V_TIPO_DOC_EQUIV FROM PCLUB.ADMPT_TIPO_DOC WHERE admpv_cod_equiv=C_TIPO_DOC;

    --Valido si el ClienteIB no tenga ninguna linea asociada
    SELECT NVL(COUNT(*),0) INTO V_CONT_1 FROM  PCLUB.ADMPT_CLIENTEIB VIB
    WHERE (VIB.ADMPV_NUM_LINEA ='' or VIB.ADMPV_NUM_LINEA is null) AND (VIB.Admpv_Cod_Cli = '' or VIB.Admpv_Cod_Cli is null)
    AND VIB.ADMPV_TIPO_DOC = V_TIPO_DOC_CLARO AND VIB.ADMPV_NUM_DOC = C_NUM_DOC AND VIB.ADMPC_ESTADO = 'A';

    IF V_CONT_1 <> 0 THEN
          IF V_CONT_1 < 2 THEN
          SELECT admpn_cod_cli_ib INTO V_CODCLIIB FROM  PCLUB.ADMPT_CLIENTEIB VIB
          WHERE (VIB.ADMPV_NUM_LINEA ='' or VIB.ADMPV_NUM_LINEA is null) AND (VIB.Admpv_Cod_Cli = '' or VIB.Admpv_Cod_Cli is null)
          AND VIB.ADMPV_TIPO_DOC = V_TIPO_DOC_CLARO AND VIB.ADMPV_NUM_DOC = C_NUM_DOC AND VIB.ADMPC_ESTADO = 'A';

          --Validando que la línea se encuentra en la Bd de DWH
          SELECT NVL(COUNT(*),0) INTO V_EXIST
          FROM dm.BASE_ABONADOS_CLARO@dbl_reptdm_d
          WHERE TIPO_ABONADO=1 AND ESTADO IN ('ACTIVE','ACTIVO','GRACE')
          AND MSISDN  = CONCAT('51',C_NUM_LINEA);
          ...
          IF V_EXIST <> 0 THEN
              ...
              BEGIN
              --Valido si el CLIENTE PREPAGO está en Claro Club
              SELECT NVL(COUNT(*),0) INTO V_COUNT_PRE
              ...
              IF V_COUNT_PRE = 0 THEN
                  --Si no existe en CC, Registro en la BD de Claro Club
                  --Obtengo los datos de DW para insertar en la tabla admpt_cliente
                  SELECT
                  S_FIRST_NAME,S_LAST_NAME,X_SEXO,DEPARTAMENTO,FIRSTCALLDATE,ESTADO,X_DOC_NUM,X_DOC_TYPE
                  INTO
                  V_FIRST_NAME,V_LAST_NAME,V_SEXO,V_DEPARTAMENTO,V_FIRST_CALLDATE,V_ESTADO,V_NRO_DOC_DWH,V_TIPO_DOC_DWH
                  FROM dm.BASE_ABONADOS_CLARO@dbl_reptdm_d
                  WHERE TIPO_ABONADO=1 AND ESTADO IN ( 'ACTIVE','ACTIVO','GRACE')
                  AND MSISDN  = CONCAT('51',C_NUM_LINEA) AND rownum=1;
                  ...
                  IF V_ESTADO = 'ACTIVE' OR V_ESTADO = 'ACTIVO' THEN
                     V_ESTADO := 'A';
                  ELSE
                     V_ESTADO := '';
                  END IF;
                  IF V_TIPO_DOC_CLARO = V_COD_TD_DWH AND C_NUM_DOC = V_NRO_DOC_DWH THEN

                      --Registro en ADMPT_CLIENTE

                      FOR R IN C_LINEASXCLI(V_COD_TD_DWH,C_NUM_DOC) LOOP

                          --OBTENGO los valores de dwh

                          SELECT X_DOC_NUM,X_DOC_TYPE,S_FIRST_NAME,S_LAST_NAME,X_SEXO,DEPARTAMENTO,FIRSTCALLDATE,ESTADO
                          INTO V_NRO_DOC_DWH2, V_TIPO_DOC_DWH2,V_FIRST_NAME2,V_LAST_NAME2,V_SEXO2,V_DEPARTAMENTO2,V_FIRST_CALLDATE2,V_ESTADO2
                          FROM dm.BASE_ABONADOS_CLARO@dbl_reptdm_d
                          WHERE TIPO_ABONADO=1 AND ESTADO IN ('ACTIVE','ACTIVO','GRACE')
                          AND MSISDN  = R.MSISDN  and rownum=1;
                          ...
                          IF V_ESTADO2 = 'ACTIVE' OR V_ESTADO2 = 'ACTIVO' OR V_ESTADO2 = 'GRACE' THEN
                             V_ESTADO2 := 'A';
                          ELSE
                             V_ESTADO2 := '';
                          END IF;
                      ...
                      insert into PCLUB.admpt_cliente
                        (admpv_cod_cli, admpv_cod_segcli, admpn_cod_catcli, admpv_tipo_doc, admpv_num_doc, admpv_nom_cli, admpv_ape_cli, admpc_sexo, admpv_est_civil, admpv_email, admpv_prov, admpv_depa, admpv_dist, admpd_fec_activ, admpv_cicl_fact, admpc_estado, admpv_cod_tpocl)
                      values
                              (VL_NUMLINEA, '', '2', V_COD_TD_DWH2, V_NRO_DOC_DWH2, V_FIRST_NAME2, V_LAST_NAME2, V_SEXO2, '', '', '', V_DEPARTAMENTO2, '', V_FIRST_CALLDATE2, '', V_ESTADO2, '3');
                      COMMIT;
                          END IF;

                          IF VL_NUMLINEA=C_NUM_LINEA THEN
                      --Asocio al clienteIB con el clienteCC
                      UPDATE PCLUB.admpt_clienteib
                      SET   admpv_num_linea = C_NUM_LINEA,
                            admpv_cod_cli   = C_NUM_LINEA
                      WHERE admpn_cod_cli_ib = V_CODCLIIB;
                      COMMIT;
                      ...
                      IF V_EXIST = 0 THEN
                        ...
                        COMMIT;
                      ELSE
                      --ACTUALIZAR EL CODIGO DEL CLIENTE CC EN EL SALDO
                        UPDATE PCLUB.admpt_saldos_cliente
                                SET  ADMPV_COD_CLI = C_NUM_LINEA,
                                     admpc_estpto_ib ='A',
                                     admpc_estpto_cc ='A'
                              WHERE ADMPN_COD_CLI_IB = V_CODCLIIB;
                        COMMIT;
                      END IF;
                            --ACTUALIZO EN EL KARDEX
                      ...
                      IF V_EXIST = 0 THEN
                        INSERT INTO PCLUB.admpt_saldos_cliente (admpn_id_saldo, admpv_cod_cli,admpn_cod_cli_ib,admpn_saldo_cc,admpn_saldo_ib,admpc_estpto_cc,admpc_estpto_ib)
                              VALUES (PCLUB.admpt_sld_cl_sq.nextval, VL_NUMLINEA, '', 0, 0,'A', NULL );
                        COMMIT;
                      END IF;
                   END IF;
                      END LOOP;
                   -- REGISTRO EN LA TABLA ADMPT_IMP_ENT_BON_PRE
                      INSERT INTO PCLUB.ADMPT_IMP_ENT_BON_PRE(ADMPN_ID_FILA,ADMPV_COD_CLI,ADMPD_FEC_OPER,ADMPD_FEC_TRANS,ADMPV_MSJE_ERROR,ADMPD_COD_CLI_IB,ADMPD_NUM_DOC)
                      VALUES(PCLUB.ADMPT_IMP_ENT_BON_PRE_SQ.NEXTVAL,C_NUM_LINEA,V_FECHA, TO_DATE(TO_CHAR(SYSDATE,'DD/MM/YYYY HH:MI PM'),'DD/MM/YYYY HH:MI PM'),'',V_CODCLIIB,C_NUM_DOC);
                   COMMIT;
              ELSE
                      --REGISTRA EN EL IMP NO SE PROCESO POR QUE SON DIFERENTE PROPIETARIOS
                      INSERT INTO PCLUB.ADMPT_IMP_ENT_BON_PRE(ADMPN_ID_FILA,ADMPV_COD_CLI,ADMPD_FEC_OPER,ADMPD_FEC_TRANS,ADMPV_MSJE_ERROR,ADMPD_COD_CLI_IB,ADMPD_NUM_DOC)
                      VALUES(PCLUB.ADMPT_IMP_ENT_BON_PRE_SQ.NEXTVAL,C_NUM_LINEA,V_FECHA, TO_DATE(TO_CHAR(SYSDATE,'DD/MM/YYYY HH:MI PM'),'DD/MM/YYYY HH:MI PM'),'NO SE PROCESO PORQUE EL CLIENTE IB ENVIO UNA LÍNEA QUE NO LE PERTENECE',V_CODCLIIB,C_NUM_DOC);
                      COMMIT;
                   END IF;
              ELSE
                   BEGIN
                   --Si el cliente CC ya se encuentra registrado en la BD de CC
                   --SELECT nvl(admpv_cod_tpdoc,'-1') INTO V_TIPO_DOC_CLARO FROM admpt_tipo_doc WHERE admpv_dsc_docum=V_TIPO_DOC_EQUIV;
                   SELECT C.ADMPV_TIPO_DOC,C.ADMPV_NUM_DOC INTO V_TIPODOC_CC,V_NRODOC_CC
                   FROM  PCLUB.ADMPT_CLIENTE C
                   WHERE C.ADMPV_COD_CLI=C_NUM_LINEA
                   ...
                   IF TRIM(V_TIPO_DOC_CLARO) = TRIM(V_TIPODOC_CC) AND TRIM(C_NUM_DOC) = TRIM(V_NRODOC_CC) THEN
                      ...
                      WHERE admpn_cod_cli_ib = V_CODCLIIB;
                      COMMIT;
                      --OBTENER EL SALDO DEL CLIENTE IB POR CODIGO DEL CLIENTE IB (PUNTO A)
                      BEGIN
                      SELECT admpn_saldo_ib  INTO V_SALDO_IB
                      FROM PCLUB.admpt_saldos_cliente A
                      WHERE A.ADMPN_COD_CLI_IB = V_CODCLIIB;
                      EXCEPTION
                          WHEN NO_DATA_FOUND THEN
                          V_SALDO_IB := 0;
                      END;
                      -- ELIMINAR EL REGISTRO DEL SALDO DEL CLIENTE IB
                      DELETE FROM PCLUB.admpt_saldos_cliente A
                      WHERE A.ADMPN_COD_CLI_IB = V_CODCLIIB;
                      COMMIT;
                      --INSERTO SALDO
                     SELECT COUNT(*) INTO V_EXIST
                      ...
                      IF V_EXIST > 0 THEN
                       ...
                       SET ADMPN_COD_CLI_IB = V_CODCLIIB,
                           ADMPC_ESTPTO_CC  = 'A',
                            ADMPC_ESTPTO_IB  = 'A',
                            admpn_saldo_ib   = V_SALDO_IB --ACTUALIZAR EL SALDO DEL CLIENTE IB OBTENIDO DEL PUNTO A
                        WHERE ADMPV_COD_CLI = C_NUM_LINEA;
                        COMMIT;
                      ELSE

                        --INSERTO EL SALDO DEL CLIENTE IB DEL PUNTO A
                        INSERT INTO PCLUB.admpt_saldos_cliente (admpn_id_saldo, admpv_cod_cli,admpn_cod_cli_ib,admpn_saldo_cc,admpn_saldo_ib,admpc_estpto_cc,admpc_estpto_ib)
                        VALUES (PCLUB.admpt_sld_cl_sq.nextval, C_NUM_LINEA, V_CODCLIIB, 0, V_SALDO_IB,'A', 'A' );
                        COMMIT;
                      END IF;
                      --ACTUALIZO EN EL KARDEX
                      UPDATE PCLUB.ADMPT_KARDEX
                        ...
                        WHERE ADMPN_COD_CLI_IB = V_CODCLIIB;
                      COMMIT;
                      -- Inserta registro en la tabla ADMPT_IMP_ENT_BON_PRE
                      INSERT INTO PCLUB.ADMPT_IMP_ENT_BON_PRE(ADMPN_ID_FILA,ADMPV_COD_CLI,ADMPD_FEC_OPER,ADMPD_FEC_TRANS,ADMPV_MSJE_ERROR,ADMPD_COD_CLI_IB,ADMPD_NUM_DOC)
                      VALUES(PCLUB.ADMPT_IMP_ENT_BON_PRE_SQ.NEXTVAL,C_NUM_LINEA,V_FECHA, TO_DATE(TO_CHAR(SYSDATE,'DD/MM/YYYY HH:MI PM'),'DD/MM/YYYY HH:MI PM'),'',V_CODCLIIB,C_NUM_DOC);
                      COMMIT;
                    ELSE
                        --REGISTRA EN EL IMP NO SE PROCESO POR QUE EXISTE EL CLIENTE IB Y EL CLIENTE CC Y SON DIFERENTE PERSONAS
                        INSERT INTO PCLUB.ADMPT_IMP_ENT_BON_PRE(ADMPN_ID_FILA,ADMPV_COD_CLI,ADMPD_FEC_OPER,ADMPD_FEC_TRANS,ADMPV_MSJE_ERROR,ADMPD_COD_CLI_IB,ADMPD_NUM_DOC)
                        VALUES(PCLUB.ADMPT_IMP_ENT_BON_PRE_SQ.NEXTVAL,C_NUM_LINEA,V_FECHA, TO_DATE(TO_CHAR(SYSDATE,'DD/MM/YYYY HH:MI PM'),'DD/MM/YYYY HH:MI PM'),'EL CLIENTE IB ENVIO UNA LÍNEA QUE NO LE PERTENECE',V_CODCLIIB,C_NUM_DOC);
                        COMMIT;
                    END IF;
                 EXCEPTION
                   WHEN NO_DATA_FOUND THEN
                        if V_CODCLIIB =-1 then
                           INSERT INTO PCLUB.ADMPT_IMP_ENT_BON_PRE(ADMPN_ID_FILA,ADMPV_COD_CLI,ADMPD_FEC_OPER,ADMPD_FEC_TRANS,ADMPV_MSJE_ERROR,ADMPD_COD_CLI_IB,ADMPD_NUM_DOC)
                           VALUES(PCLUB.ADMPT_IMP_ENT_BON_PRE_SQ.NEXTVAL,C_NUM_LINEA,V_FECHA, TO_DATE(TO_CHAR(SYSDATE,'DD/MM/YYYY HH:MI PM'),'DD/MM/YYYY HH:MI PM'),'No se encontró la secuencia admpt_cli_ib_sq.nextval',V_CODCLIIB,C_NUM_DOC);
                           COMMIT;
                         else
                             if V_TIPO_DOC_CLARO ='-1' or V_TIPO_DOC_CLARO is null or V_TIPO_DOC_EQUIV='-1' or V_TIPO_DOC_EQUIV is null then
                               INSERT INTO PCLUB.ADMPT_IMP_ENT_BON_PRE(ADMPN_ID_FILA,ADMPV_COD_CLI,ADMPD_FEC_OPER,ADMPD_FEC_TRANS,ADMPV_MSJE_ERROR,ADMPD_COD_CLI_IB,ADMPD_NUM_DOC)
                               VALUES(PCLUB.ADMPT_IMP_ENT_BON_PRE_SQ.NEXTVAL,C_NUM_LINEA,V_FECHA, TO_DATE(TO_CHAR(SYSDATE,'DD/MM/YYYY HH:MI PM'),'DD/MM/YYYY HH:MI PM'),'No se encontró el tipo de documento equivalente en la tabla admpt_tipo_doc',V_CODCLIIB,C_NUM_DOC);
                               COMMIT;
                             ELSE
                               INSERT INTO PCLUB.ADMPT_IMP_ENT_BON_PRE(ADMPN_ID_FILA,ADMPV_COD_CLI,ADMPD_FEC_OPER,ADMPD_FEC_TRANS,ADMPV_MSJE_ERROR,ADMPD_COD_CLI_IB,ADMPD_NUM_DOC)
                                 VALUES(PCLUB.ADMPT_IMP_ENT_BON_PRE_SQ.NEXTVAL,C_NUM_LINEA,V_FECHA, TO_DATE(TO_CHAR(SYSDATE,'DD/MM/YYYY HH:MI PM'),'DD/MM/YYYY HH:MI PM'),V_ERROR,V_CODCLIIB,C_NUM_DOC);
                               COMMIT;
                              end if;
                         end if;
                    WHEN OTHERS THEN
           ...
           ELSE
             INSERT INTO PCLUB.ADMPT_IMP_ENT_BON_PRE(ADMPN_ID_FILA,ADMPV_COD_CLI,ADMPD_FEC_OPER,ADMPD_FEC_TRANS,ADMPV_MSJE_ERROR,ADMPD_COD_CLI_IB,ADMPD_NUM_DOC)
             VALUES(PCLUB.ADMPT_IMP_ENT_BON_PRE_SQ.NEXTVAL,C_NUM_LINEA,V_FECHA, TO_DATE(TO_CHAR(SYSDATE,'DD/MM/YYYY HH:MI PM'),'DD/MM/YYYY HH:MI PM'),'NO ES PREPAGO CLARO',V_CODCLIIB,C_NUM_DOC);
             COMMIT;
           END IF;
    ELSE
           INSERT INTO PCLUB.ADMPT_IMP_ENT_BON_PRE(ADMPN_ID_FILA,ADMPV_COD_CLI,ADMPD_FEC_OPER,ADMPD_FEC_TRANS,ADMPV_MSJE_ERROR,ADMPD_COD_CLI_IB,ADMPD_NUM_DOC)
           VALUES(PCLUB.ADMPT_IMP_ENT_BON_PRE_SQ.NEXTVAL,C_NUM_LINEA,V_FECHA, TO_DATE(TO_CHAR(SYSDATE,'DD/MM/YYYY HH:MI PM'),'DD/MM/YYYY HH:MI PM'),'NO SE PROCESO PORQUE CLIENTE IB ESTA DUPLICADO',V_CODCLIIB,C_NUM_DOC);
           COMMIT;
         END IF;
    ELSE
           INSERT INTO PCLUB.ADMPT_IMP_ENT_BON_PRE(ADMPN_ID_FILA,ADMPV_COD_CLI,ADMPD_FEC_OPER,ADMPD_FEC_TRANS,ADMPV_MSJE_ERROR,ADMPD_COD_CLI_IB,ADMPD_NUM_DOC)
           VALUES(PCLUB.ADMPT_IMP_ENT_BON_PRE_SQ.NEXTVAL,C_NUM_LINEA,V_FECHA, TO_DATE(TO_CHAR(SYSDATE,'DD/MM/YYYY HH:MI PM'),'DD/MM/YYYY HH:MI PM'),'NO SE PROCESO PORQUE CLIENTE IB NO EXISTE',V_CODCLIIB,C_NUM_DOC);
           COMMIT;
    END IF;
        -- Fin del Bloque
    <<NEXT_CLIENTEIB>>
   ...
   CLOSE CLIENTEIB;
   ---FIN
  --CARGA DE LOS VALORES K_NUMREGTOT,K_NUMREGPRO Y K_NUMREGERR

  SELECT COUNT(*) INTO K_NUMREGTOT
  FROM PCLUB.ADMPT_IMP_ENT_BON_PRE;
  --WHERE ADMPD_FEC_OPER=V_FECHA;

  SELECT COUNT(*) INTO K_NUMREGPRO
  FROM PCLUB.ADMPT_IMP_ENT_BON_PRE
  WHERE (admpv_msje_error ='' or admpv_msje_error is null) ;

  SELECT COUNT(*) INTO K_NUMREGERR
  FROM PCLUB.ADMPT_IMP_ENT_BON_PRE
  WHERE (admpv_msje_error !='' or admpv_msje_error is not null);

EXCEPTION
  WHEN others then
   K_CODERROR:=SQLCODE;
   K_DESCERROR:=SUBSTR(SQLERRM,1,400);
END ADMPSI_ENT_BON_PRE;
...
BEGIN
  /*genera secuencial de kardex*/
    ...
    ELSE
            ADMPSI_DSCTO_PUNTO(K_CODCLIENTEIB,K_CODCLIENTECC, K_PUNTOSIGNO*-1, K_CODCONCEPTO, K_CODCONCEPTO,K_CODERROR,K_MSJERROR);
  ...
  WHILE CLIENTEIBF %FOUND LOOP
   ...
   /* verifica si el registro no existe en la tabla auxiliar*/
     ...
     /* verifica el signo de los puntos y cambio el tipo de operacion(entrada/salida)*/
       ...
       /* busca el codigoClaro del clienteIB*/
         ...
         /* verifico el estado de los puntos*/
          ...
          ELSE
            BEGIN
              UPDATE PCLUB.ADMPT_TMP_FACADE_IB R
              SET R.ADMPC_COD_ERROR='17'
                  ,R.ADMPV_MSJE_ERROR='El cliente IB existe, pero el saldo del cliente no existe en la tabla saldos.'
              WHERE ADMPV_TIPO_DOC=C_TIPODOCUMENTO
                 AND ADMPV_NUM_DOC=C_NUMDOCUMENTO
                 AND ADMPD_FEC_OPER=C_FECPROCESO
                 AND ADMPV_NOM_ARCH=C_NOMARCHIVO
     ...
     ELSE
       BEGIN
          UPDATE PCLUB.ADMPT_TMP_FACADE_IB R
          SET R.ADMPC_COD_ERROR='16'
              ,R.ADMPV_MSJE_ERROR='El cliente IB no existe, no se le puede entregar puntos.'
          WHERE ADMPV_TIPO_DOC=C_TIPODOCUMENTO
             AND ADMPV_NUM_DOC=C_NUMDOCUMENTO
             AND ADMPD_FEC_OPER=C_FECPROCESO
             AND ADMPV_NOM_ARCH=C_NOMARCHIVO
...
SELECT ADMPN_ID_FILA, ADMPV_NUM_LINEA, ADMPN_SN_CODE, ADMPN_SP_CODE, ADPMV_ACCION
 ...
 WHERE ADMPD_FEC_REG < TRUNC(K_FECHAPROC) + 1 AND
       REPLACE(ADMPV_NUM_LINEA,' ','') IS NOT NULL AND
       ADMPC_ESTADO = 'P';
END;
...
begin
  ...
  EXCEPTION
   ...
   if (K_CODERRORP=42 or K_CODERRORP=43) then
        K_DESCERROR:= K_DESCERRORP;
     else
...
CURSOR CONCEPTOS IS
  ...
  WHERE ADMPC_ESTADO IN ('A')
        ...
        AND ( ADMPV_DESC <>'DEBITO AUTOMATICO TC'
        AND ADMPV_DESC <>'BONO CADUCADO')
        AND ADMPC_TPO_PUNTO <> 'I';
...
CURSOR PUNTOSVENCC(cod_concepto varchar2,tipo_punto char,vigencia number) IS
  ...
  WHERE ( k.ADMPD_FEC_TRANS > dFECHA_0 AND k.ADMPD_FEC_TRANS < dFECHA_1 )
...
BEGIN
  ...
  WHILE CONCEPTOS %FOUND LOOP
   ...
   IF (V_VIGENCIA >0 ) THEN
     dFECHA_0 := TRUNC(ADD_MONTHS(dFECHA_REG_VEN, - V_VIGENCIA), 'MM')+(1 / (24 * 60 * 60))-(2 / (24 * 60 * 60));
     dFECHA_1 := TRUNC(LAST_DAY(ADD_MONTHS(dFECHA_REG_VEN, -V_VIGENCIA)))+(1-(1 / (24 * 60 * 60)));
       ...
       BEGIN
         ...
         BEGIN
            ...
            INSERT INTO PCLUB.admpt_kardex (ADMPN_ID_KARDEX, ADMPN_COD_CLI_IB, ADMPV_COD_CLI,
                                    ADMPV_COD_CPTO,ADMPD_FEC_TRANS,ADMPN_PUNTOS,
                                    ADMPV_NOM_ARCH, ADMPC_TPO_OPER, ADMPC_TPO_PUNTO,
                                    ADMPN_SLD_PUNTO, ADMPC_ESTADO, admpd_fec_reg, admpv_usu_reg)
            VALUES(nKARDEX, V_CODCLIB, V_CODCC,V_CONCEPTO,
                   dFECHA_REG_VEN,V_SUMA*(-1),null,'S','C',0,'V', sysdate, K_USUARIO);
            ...
            WHERE  ADMPV_COD_CLI=V_CODCC
                   AND ( ADMPD_FEC_TRANS > dFECHA_0 AND ADMPD_FEC_TRANS < dFECHA_1 )
...
C_TIP_DOC VARCHAR2(40);
C_NUM_DOC VARCHAR2(40);
C_CO_ID VARCHAR2(40);
C_CI_FAC VARCHAR2(20);
C_COD_TIP_CL VARCHAR2(20);
C_TIP_CL VARCHAR2(20);
...
IF (K_TIPCLI IS NULL)  OR (K_TIPCLI <> '2'  AND K_TIPCLI <> '3')THEN
...
IF K_TIPCLI = '3' THEN--PREPAGO
    ...
    AND C.ADMPV_TIPO_DOC=K_TIPO_DOC
    AND C.ADMPV_NUM_DOC=K_NUM_DOC;

    IF V_REG > 0 THEN

        OPEN CURSORCLI FOR
        SELECT C.ADMPV_COD_CLI LINEA
        FROM PCLUB.ADMPT_CLIENTE C
        WHERE C.ADMPV_COD_TPOCL = '3'
        AND C.ADMPV_TIPO_DOC=K_TIPO_DOC
        AND C.ADMPV_NUM_DOC=K_NUM_DOC;
...
PROCEDURE ADMPSI_DSCTO_PUNTO(K_COD_CLIENTE IN VARCHAR2,K_COD_CLIENTECC IN VARCHAR2, K_PUNTOS IN NUMBER, K_CONCEPTOCC IN VARCHAR2, K_CONCEPTOIB IN VARCHAR2,K_CODERROR OUT NUMBER, K_MSJERROR OUT VARCHAR2)
IS
...
SELECT KA.ADMPC_TPO_PUNTO, KA.ADMPN_ID_KARDEX, KA.ADMPN_SLD_PUNTO,  KA.ADMPV_COD_CLI
FROM PCLUB.ADMPT_KARDEX KA
WHERE KA.ADMPC_ESTADO='A'
AND KA.ADMPC_TPO_OPER='E'
AND KA.ADMPN_SLD_PUNTO>0
AND KA.ADMPD_FEC_TRANS<=TRUNC(SYSDATE)
AND KA.ADMPN_COD_CLI_IB= K_COD_CLIENTE
AND ADMPC_TPO_PUNTO='I'
ORDER BY ADMPN_ID_KARDEX ASC;
...
V_PUNTOS_REQUERIDOS:=K_PUNTOS;
   ...
   IF K_COD_CLIENTE IS NOT NULL THEN
           ...
           OPEN LISTA_KARDEX_3;
           FETCH LISTA_KARDEX_3 INTO LK_TPO_PUNTO, LK_ID_KARDEX, LK_SLD_PUNTOS, LK_COD_CLI;
              ...
              IF LK_SLD_PUNTOS<=V_PUNTOS_REQUERIDOS THEN
                ...
                -- Actualiza Saldos_cliente
                  IF LK_TPO_PUNTO='I' THEN
                     ...
                     VALUES(ADMPT_KARDEX_SQ.NEXTVAL,K_COD_CLIENTE,K_COD_CLIENTECC,K_CONCEPTOIB,TO_DATE(TO_CHAR(SYSDATE,'dd/mm/yyyy'),'dd/mm/yyyy'),
                     ...
                     UPDATE PCLUB.ADMPT_SALDOS_CLIENTE
                         SET
                             ADMPN_SALDO_IB = - LK_SLD_PUNTOS + (SELECT NVL(ADMPN_SALDO_IB,0) FROM PCLUB.ADMPT_SALDOS_CLIENTE
                                      WHERE ADMPN_COD_CLI_IB=K_COD_CLIENTE)
                       WHERE ADMPN_COD_CLI_IB = K_COD_CLIENTE;

                       LK_SLDDESC:=LK_SLDDESC+LK_SLD_PUNTOS;
                   ...
                   WHERE ADMPN_ID_KARDEX = LK_ID_KARDEX;
                      ...
                      IF LK_TPO_PUNTO='I' THEN
                         ...
                         UPDATE PCLUB.ADMPT_SALDOS_CLIENTE
                           ...
                           LK_SLDDESC:=LK_SLDDESC+V_PUNTOS_REQUERIDOS;
        ...
        IF LK_SLDDESC>0 THEN
         INSERT INTO PCLUB.ADMPT_KARDEX(ADMPN_ID_KARDEX,ADMPN_COD_CLI_IB,ADMPV_COD_CLI,ADMPV_COD_CPTO
                         ,ADMPD_FEC_TRANS,ADMPN_PUNTOS,ADMPC_TPO_OPER,ADMPC_TPO_PUNTO,ADMPN_SLD_PUNTO,ADMPC_ESTADO)
                         VALUES(PCLUB.ADMPT_KARDEX_SQ.NEXTVAL,K_COD_CLIENTE,K_COD_CLIENTECC,K_CONCEPTOIB,TO_DATE(TO_CHAR(SYSDATE,'dd/mm/yyyy'),'dd/mm/yyyy'),
                         LK_SLDDESC * (-1),'S','I',0,'A');
       END IF;
