#! /bin/ksh
#*************************************************************
#* DESCRIPCION           : Puntos por Regularización TFI
#* EJECUCION             : Control-D                            
...
if [ "$FILEDATA" = "" ] ; then
   ...
   pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de El Archivo de Regularización TFI no se encuentra en la ruta" 
...
if [ $CANT_DATA = 0 ] ; then
	...
	pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de El Archivo de Regularización TFI se encuentra vacio." 
	echo "Buen día, los siguientes archivos $ARCHNAME en la carpeta de ORIGEN no contienen datos." $'\n' "Favor de atender este inconveniente" $'\n' " Gracias." $'\n'  | mail -s "El Archivo de Regularización TFI se encuentra vacio" $IT_MAIL
...
if [ "$promo" == "" ] || [ "$user" == "" ] ; then
   ...
   pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de El Archivo de Regularización TFI no se encuentra en forma Correcta" 
   pMessage "Termino subproceso"
   pMessage "************************************" 	    
   pMessage "************************************"
   cat $FILELOG >> $FILELOGPR
   ...
   ARCHIVO=`echo $ARCHNAME | awk 'BEGIN{FS="Regularizacion/"} {printf $2 }'`
	...
	if [ "$FILEDATATEMP" = "" ] || [ $CANT_DATATEMP = 0 ] ; then
	   ...
	   pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de El Archivo de Regularización TFI no contiene datos válidos" 
	   echo "Buen día, el archivo $NomArchivo en la carpeta $RUTANAME no contiene datos validos a procesar." $'\n' "Favor de atender este inconveniente" $'\n' "Gracias."$'\n'  | mail -s "TFI: El Archivo de Regularización TFI no contiene datos válidos." $IT_MAIL 
	   #pMessage "Ruta del Archivo log : " $FILELOG
	   pMessage "Termino subproceso"
	   pMessage "************************************" 	    
	   pMessage "************************************" 
	...
	pMessage "Se procede a importar los datos del archivo de entrada a la tabla de Regularización TFI"
	...
	if [ $VALIDAT_CTL -ne 0 ] ;
		...
		echo "Buen día, ocurrio un error al momento de importar los datos del archivo $BAD a la tabla de Regularización TFI." $'\n' "Favor de atender este inconveniente" $'\n' " Gracias." $'\n'  | mail -s "Error al importar datos" $IT_MAIL
		pMessage "Termino subproceso"
	        pMessage "************************************" 	    
		pMessage "************************************" 
		echo $'\n'
...
EOP
	...
	if [ ${VALIDA_EJEC_SP} -ne 0 ] || [ ${VALIDA_EJEC_SP_B} -ne 0 ] ; then
		pMessage "Hora y Fecha: $demora"
		pMessage "Hubo un error durante la ejeción del SP PKG_CC_PTOSTFI.ADMPSI_REGULARIZACION_TFI" 
		pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de IMPORTACION DE PUNTOS POR REGULARIZACION TFI – Se encontraron errores" 
		echo "Buen día, ocurrio un problema al ejecutar el SP de PKG_CC_PTOSTFI.ADMPSI_REGULARIZACION_TFI." $' \n' "Favor de atender este inconveniente" $'\n' "Gracias." $'\n'  | mail -s "IMPORTACION DE PUNTOS POR REGULARIZACION TFI – Se encontraron errores" $IT_MAIL 		
		pMessage "Se envió correo Fecha y Hora: $demora" 		
		pMessage "Termino subproceso"
	    pMessage "************************************" 	    
		pMessage "************************************" 
...
pMessage "Termino subproceso Asignación de Puntos por Regularización TFI ClaroClub"
pMessage "********** FINALIZANDO SUBPROCESO ********** " 
pMessage "Fin de subproceso "
pMessage "************************************" 
#pMessage "Ruta del Archivo log : ${FILELOG}" 
