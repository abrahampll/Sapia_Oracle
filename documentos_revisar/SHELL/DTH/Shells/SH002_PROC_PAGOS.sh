...
if [ $VALIDAT_CTL -ne 0 ] ;
	...
	#echo "Buen día, ocurrio un error al momento de importar los datos a la tabla de pagos.Archivo BAD: $BAD" $'\n' "Favor de atender este inconveniente" $'\n' " Gracias." $'\n'  | mail -s "DTH-POSTPAGO: Error BSCS." $IT_MAIL
    BAD_ZIP=importaPago_BAD_$FECHA.zip
	BAD_NOM=importaPago_BAD_$FECHA.bad
	cd $DIRFALLOSDTH
	zip $BAD_ZIP $BAD_NOM
	(echo "Buen día, ocurrio un error al momento de importar los datos a la tabla de pagos. Se adjunta el archivo BAD." $'\n' "Favor de atender este inconveniente" $'\n' " Gracias." $'\n' ; uuencode $BAD_ZIP $BAD_ZIP) | mail -s "DTH-POSTPAGO: Error DATA BSCS." $IT_MAIL
    rm -f $BAD_ZIP
	cd $DIR_DTH_SHELL
	#pMessage "Termino proceso"
    pMessage "************************************" 
...
if [ ${VALIDA_EJEC_SP} -ne 0 ] || [ ${VALIDA_EJEC_SP_B} -ne 0 ] ; then
    pMessage "Hora y Fecha: $demora"
    pMessage "Hubo un error durante la ejeción del SP PKG_CC_PTOSFIJA.ADMPSI_FACTURADTH" 
    pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de IMPORTACION DE PAGOS – Se encontraron errores" 
    echo "Buen día, ocurrio un problema al ejecutar el SP de PKG_CC_PTOSFIJA.ADMPSI_FACTURADTH." $' \n' "Favor de atender este inconveniente" $'\n' "Gracias." $'\n'  | mail -s "DTH-POSTPAGO: IMPORTACION DE PAGOS " $IT_MAIL 		
    pMessage "Se envió correo Fecha y Hora: $demora"    
	exit
fi
...
if [ ${VALIDA_EJEC_SP} -ne 0 ] || [ ${VALIDA_EJEC_SP_B} -ne 0 ] ; then
    ...
    pMessage "Hora y Fecha: $demora"
    pMessage "Hubo un error durante la ejeción del SP PKG_CC_PTOSFIJA.ADMPSI_EFACTURADTH"  
	echo "Buen día, Hubo un error durante la ejeción del SP PKG_CC_PTOSFIJA.ADMPSI_FACTURADTH." $'\n' "Favor de atender este inconveniente" $'\n' "Gracias."$'\n'  | mail -s "DTH-POSTPAGO: Importacion de Pagos." $IT_MAIL 
	exit
else
	if [ $CANT_DATA != 0 ] ; then
		pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de Errores en el proceso de facturacion" 
...
pMessage "Se finalizó el proceso de Ejecución de ADMPSI_EFACTURADTH"

pMessage "Se finalizó el proceso de Ejecución de ADMPSI_FACTURADTH"
