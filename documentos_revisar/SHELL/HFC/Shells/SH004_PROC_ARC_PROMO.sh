#! /bin/ksh
#*************************************************************
#* DESCRIPCION           : Puntos por Promocion HFC
#* EJECUCION             : Control-D                            
#* AUTOR                 :                                   
...
CONTROL=$DIRCONTROLHFC/importaPPromHFC.ctl
BAD=$DIRFALLOSHFC/importaPPromHFC_BAD_$FECHA.bad
FILELOG=$DIRLOGHFC/SH004_PROC_ARC_PROMO_$FECHA.log
CTL_LOG=$DIRLOGHFC/CTL004_LOG_$FECHA.log
...
if [ "$FILEDATA" = "" ] ; then
   ...
   pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de El Archivo de Promocion HFC no se encuentra en la ruta" 
   echo "Buen día, no se encontraron los siguientes archivos $ARCHNAME en la carpeta de $RUTANAME." $'\n' "Favor de atender este inconveniente" $'\n' "Gracias."$'\n'  | mail -s "El Archivo de Promocion HFC no se encuentra en la ruta." $IT_MAIL 
...
if [ $CANT_DATA = 0 ] ; then
	...
	pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de El Archivo de Promocion HFC se encuentra vacio." 
	echo "Buen día, los siguientes archivos $ARCHNAME en la carpeta de ORIGEN no contienen datos." $'\n' "Favor de atender este inconveniente" $'\n' " Gracias." $'\n'  | mail -s "El Archivo de Promocion HFC se encuentra vacio" $IT_MAIL
...
if [ "$promo" == "" ] || [ "$user" == "" ] ; then
   ...
   TIPOCLIE="7"
	...
	if [ "$FILEDATATEMP" = "" ] || [ $CANT_DATATEMP = 0 ] ; then
	   ...
	   pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de El Archivo de Promocion HFC no contiene datos validos" 
	   echo "Buen día, el archivo $NomArchivo en la carpeta $RUTANAME no contiene datos validos a procesar." $'\n' "Favor de atender este inconveniente" $'\n' "Gracias."$'\n'  | mail -s "HFC : El Archivo de Promocion HFC no contiene datos validos." $IT_MAIL 
	   #pMessage "Ruta del Archivo log : " $FILELOG
	   pMessage "Termino subproceso"
	   pMessage "************************************" 	    
	   pMessage "************************************" 
	...
	pMessage "Se procede a importar los datos del archivo de entrada a la tabla de promocion HFC"
	...
	if [ $VALIDAT_CTL -ne 0 ] ;
		...
		echo "Buen día, ocurrio un error al momento de importar los datos del $NomArchivo a la tabla de promocion HFC." $'\n' "Favor de atender este inconveniente" $'\n' " Gracias." $'\n'  | mail -s "Error al importar datos" $IT_MAIL
		cat $CTL_LOG >> $FILELOG
		rm -f $CTL_LOG 
		pMessage "Termino subproceso"
	    pMessage "************************************" 	    
	...
	pMessage "Se procede a ejecutar el SP PKG_CC_PTOSFIJA.ADMPSI_PROMDTH_HFC"
...
BEGIN
	...
	begin
		...
		$PCLUB_OW.PKG_CC_PTOSFIJA.ADMPSI_PROMDTH_HFC('7',v_usuario,v_fecha,'$NomArchivo',
...
EOP
	...
	if [ ${VALIDA_EJEC_SP} -ne 0 ] || [ ${VALIDA_EJEC_SP_B} -ne 0 ] ; then
		pMessage "Hora y Fecha: $demora"
		pMessage "Hubo un error durante la ejeción del SP PKG_CC_PTOSFIJA.ADMPSI_PROMDTH_HFC" 
		pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de IMPORTACION DE PUNTOS POR PROMOCION HFC – Se encontraron errores" 
		echo "Buen día, ocurrio un problema al ejecutar el SP de PKG_CC_PTOSFIJA.ADMPSI_PROMDTH_HFC." $' \n' "Favor de atender este inconveniente" $'\n' "Gracias." $'\n'  | mail -s "IMPORTACION DE PUNTOS POR PROMOCION HFC – Se encontraron errores" $IT_MAIL 		
		pMessage "Se envió correo Fecha y Hora: $demora"    
		pMessage "Termino subproceso"
	    pMessage "************************************" 	    
		pMessage "************************************" 
	...
	if [ "$FINDFILE" = "" ] && [ "$FINDBKP" = "" ] ; then
		...
		pMessage "No existe ninguno de estos archivos $ARCHNAME" 
		pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de El Archivo de Promocion HFC no se encuentra en la ruta" 
		echo "Buen día, no se encontraron los siguientes archivos $ARCHNAME en la carpeta de $RUTANAME." $'\n' "Favor de atender este inconveniente" $'\n' "Gracias."$'\n'  | mail -s "HFC: PROMOCION, no se encuentra el archivo." $IT_MAIL 
		pMessage "Se envió correo Fecha y Hora: $demora"		
		pMessage "Termino subproceso"
	        pMessage "************************************" 	    
		pMessage "************************************"
...
pMessage "Termino subproceso Asignacion de Puntos por Promocion HFC ClaroClub"
pMessage "********** FINALIZANDO SUBPROCESO ********** " 
pMessage "Fin de subproceso "
pMessage "************************************" 
#pMessage "Ruta del Archivo log : ${FILELOG}" 
