#!/bin/sh -x
#*************************************************************
#Programa        : SH003_BONO_FID6M
#Descripción     : Entrega de bono por fidelidad 6M con validacion de unica entrega a cliente por mes.
...
USER_PROC="USRBONO6M"
PREF_SHELL="SH003_"
NOMB_SHELL="BONO_FID6M"
DESC_PROCESO="Entrega de Bono por Fidelidad 6M"
...
FILE_CTL=$DIRCTL/CTL003_${NOMB_ARCH}.ctl
...
CTRL_LOG=$DIRLOG/CTL003_${NOMB_SHELL}_$FECHA_HORA.log
EMAIL=$IT_OPERADOR
...
InicioShell(){
pMessage "-------------------------------------------------------"
pMessage "|      INICIANDO ENTREGA DE BONO POR FIDELIDAD 6M     |"
pMessage "-------------------------------------------------------"
...
FinalShell(){
pMessage "-------------------------------------------------------"
pMessage "|     FINALIZANDO ENTREGA DE BONO POR FIDELIDAD 6M    |"
pMessage "-------------------------------------------------------"
pMessage "   Fecha y Hora   |      `date +'%d-%m-%Y %H:%M:%S'`   "
pMessage "   Usuario        |      $USER_SERV                    "
pMessage "   Shell          |      $0                            "
...
$PCLUB_OW.PKG_CC_BONOS.ADMPSS_TMP_BONOFIDEL_PRE('$TIPO_BONO_FIDEL','$NOMBARCH',V_NUMREG,V_CODERROR,V_DESCERROR);
...
$PCLUB_OW.PKG_CC_BONOS.ADMPSI_ENTR_BONOFID6M_V('$NOMBARCH','$BONO','$USER_PROC',V_NUMREGTOT,V_NUMREGVAL,V_NUMREGERR,V_CODERROR,V_DESCERROR);
...
pMessage " Datos Procesados: ${FILE_NAME} ${BONO} ${FILE_PROCESO} "

ESTADO=$(ProcesarBono ${FILE_NAME} ${BONO} ${FILE_PROCESO})

if [ $ESTADO -ne 1 ] ; then
	pMessage "Hora y Fecha: `date +'%d-%m-%Y %H:%M:%S'`"
	pMessage "ERROR: Ocurrió un error en la ejecución del procedure de $DESC_PROCESO: ${PCLUB_OW}.PKG_CC_BONOS.ADMPSI_ENTR_BONOFID6M. Contacte al administrador."
	pMessage "A continuación se enviará un correo a $EMAIL con el asunto: $ASUN_MAIL"
	pMessage "Verifique el log para mayor detalle $FILE_LOG"
	pMessage "Terminó subproceso"
	pMessage "************************************" 
    echo -e "Error al ejecutar el procedure de $DESC_PROCESO: ${PCLUB_OW}.PKG_CC_BONOS.ADMPSI_ENTR_BONOFID6M.${SLDO_MAIL}" | mail -s "$ASUN_MAIL" $EMAIL
	echo $'\n'
