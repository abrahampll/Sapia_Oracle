#!/bin/sh -x
#*************************************************************
#Programa        : SH003_BONO_FID6M_SMS.sh
...
USER_PROC="$USER_BLACK"
PREF_SHELL="SH009_SMS_"
NOMB_SHELL="SH009_ENTREGA_SMS"
DESC_PROCESO="Envío de mensajes por bono"
ASUN_MAIL="ERROR: $DESC_PROCESO"
SLDO_MAIL="\n\nPor favor atender este inconveniente. \nGracias"
FLAG_PROCESO="BONO"
#TIPO_BONO_FIDEL="M"
#NOMB_ARCH="BONO_FID6M"
#EXTE_ARCH=".CCL"
IDEN_MENSAJE="MSJENTBONPLANFID6"
...
FILE_TPBONO=$DIRENTRADA/${PREF_SHELL}TIPBON_$FECHA_HORA.tmp
FILE_LOG=$DIRLOG/${NOMB_SHELL}_$FECHA_HORA.log
FILE_ERR=$DIRERROR/${NOMB_SHELL}_$FECHA_HORA.err
EMAIL=$IT_OPERADOR
...
WHENEVER SQLERROR EXIT SQL.SQLCODE;
 ...
 V_TIPOMENSA  VARCHAR2(60):='$TIPOME';

BEGIN

$PCLUB_OW.PKG_CC_ENVIO_SMS.ADMPSS_OBTENER_MENSAJE(V_TIPOMENSA,V_MENSAJE,V_CODERROR,V_DESCERROR);
...
V_TOTAL NUMBER;
V_CODERROR NUMBER;
V_IDEBONO   VARCHAR2(20);
V_DESBONO   VARCHAR2(20);
V_DESCERROR VARCHAR2(250); 
V_TIPOBONO  VARCHAR2(20);
V_DESCBONO  VARCHAR2(20);
BEGIN
...
$PCLUB_OW.PKG_CC_ENVIO_SMS.ADMPSU_IMP_BLACKLIST('$NOMBREPROC',V_TIPOBONO,V_DESCBONO,'$USER_BLACK',V_TOTAL,V_CODERROR,V_DESCERROR);
IF V_CODERROR = 0 THEN
...
$PCLUB_OW.PKG_CC_ENVIO_SMS.ADMPSS_OBT_TELEF_SMS_BONO(V_IDEBONO,V_DESBONO,'$CANTREG',V_CUR_LINEA,V_CODERROR,V_DESCERROR);
IF V_CODERROR = 0 THEN
	LOOP
...
$PCLUB_OW.PKG_CC_ENVIO_SMS.ADMPSU_ACT_ESTADOSMS_BONO($CANTREG,V_IDEBONO,V_DESBONO,'$USER_BLACK',V_CODERROR,V_DESCERROR);
...
DECLARE
  ...
  V_DBONO      VARCHAR2(20);
	V_MENSJ			 VARCHAR2(60);
  K_CODERROR   NUMBER;
  K_DESCERROR  VARCHAR2(20);
  CURS_REGIS   CURSOREF;
  BEGIN
  $PCLUB_OW.PKG_CC_ENVIO_SMS.ADMPSS_OBT_TIPOSBONO(CURS_REGIS,K_CODERROR,K_DESCERROR);
  IF K_CODERROR !=0 THEN
    DBMS_OUTPUT.PUT_LINE(TO_CHAR(K_CODERROR) ||'|' || K_DESCERROR);
  ELSE
    LOOP  
      FETCH CURS_REGIS INTO V_IDBON,V_DBONO,V_MENSJ;
      EXIT WHEN CURS_REGIS%NOTFOUND;
      DBMS_OUTPUT.PUT_LINE(TO_CHAR(V_IDBON) || '|' || V_DBONO || '|' || V_MENSJ ||'|' );
    END LOOP;
  END IF;
  CLOSE CURS_REGIS; 
EXCEPTION
...
pMessage "Proceso: Obteniendo el parámetro que indica cada cuantos teléfonos se van a enviar mensajes."
...
EstadoBonos=$(ObtenerTiposBonos $USER_BD $CLAVE_BD $SID_BD $FILE_TPBONO )
if [ $EstadoBonos -ne 1 ]; then
	pMessage "Error No se obtuvo los tipos de bonos para ser procesados."
	echo -e "Error No se obtuvo los tipos de bonos para ser procesados. Verifique el log para mayor detalle $FILE_LOG.${SLDO_MAIL}" | mail -s "$ASUN_MAIL" $EMAIL
	FinalShell
	exit
fi

while read REGISTROSBONOS 
do
	IDENTBONOS=`echo $REGISTROSBONOS | sed 's/\\r//g'|awk 'BEGIN{FS="|"} {print $1}'`
	DESCRBONOS=`echo $REGISTROSBONOS | sed 's/\\r//g'|awk 'BEGIN{FS="|"} {print $2}'`
	MENSJBONOS=`echo $REGISTROSBONOS | sed 's/\\r//g'|awk 'BEGIN{FS="|"} {print $3}'`
	IDENTBONOS=${IDENTBONOS:-X}
	DESCRBONOS=${DESCRBONOS:-X}
	
	######################################################################################
	
	ESTADO=$(ObtenerMensaje ${FILE_MENSAJ} $MENSJBONOS )
	if [ $ESTADO -ne 1 ] ; then
		pMessage "Hora y Fecha: `date +'%d-%m-%Y %H:%M:%S'`"
	...
	pMessage "Proceso: Obteniendo la cantidad de teléfonos."
	pMessage "TipoBono $IDENTBONOS $DESCRBONOS"
	ESTADO=$(TotalLineas ${FILE_TOTLIN} $IDENTBONOS $DESCRBONOS )
	...
	while [ $CONTADOR -lt $TOTAL_LINEAS ]
		do
			...
			pMessage "Proceso: Obteniendo teléfonos. Iteración: $CONTITER"
		
			pMessage "${CANTREGENV} ${IDENTBONOS} ${DESCRBONOS} ${FILE_LINEAS}"
			ESTADO=$(ProcesarLineas ${CANTREGENV} $IDENTBONOS $DESCRBONOS  $FILE_LINEAS)
			if [ $ESTADO -ne 1 ] ; then
				pMessage "Hora y Fecha: `date +'%d-%m-%Y %H:%M:%S'`"
				...
				LOGDATE=`date +"%d-%m-%Y %H:%M:%S"`
				echo "($LOGDATE) Lineas Procesadas  Cantidad  : ${CANTREGENV}"     >> $FILE_LOG
				echo "($LOGDATE) Listado de Lineas  Procesadas: ${TELEFONOLISTA}"  >> $FILE_LOG
				...
				if 	[ $EXITSTAT -ne 0 ] ; then
					...
					ESTADO=$(ActualizarEstado ${CANTREGENV} ${IDENTBONOS} ${DESCRBONOS} ${FILE_ACTEST})
					if [ $ESTADO -ne 1 ] ; then
						pMessage "Hora y Fecha: `date +'%d-%m-%Y %H:%M:%S'`"
