#! /bin/ksh
#*************************************************************
#* DESCRIPCION           : Puntos por Regularizacion DTH
#* EJECUCION             : Control-D                            
#* AUTOR                 :                                   
...
CONTROL=$DIRCONTROLDTH/importaPRegDTH.ctl
BAD=$DIRFALLOSDTH/impPRegHFC_BAD_$FECHA.bad
FILELOG=$DIRLOGDTH/SH005_PROC_ARC_REGUL_$FECHA.log
CTL_LOG=$DIRLOGDTH/CTL005_LOG_$FECHA.log
...
if [ "$FILEDATA" = "" ] ; then
   ...
   pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de El Archivo de Regularizacion DTH no se encuentra en la ruta" 
   echo "Buen día, no se encontraron los siguientes archivos $ARCHNAME en la carpeta de $RUTANAME." $'\n' "Favor de atender este inconveniente" $'\n' "Gracias."$'\n'  | mail -s "El Archivo de Regularizacion DTH no se encuentra en la ruta." $IT_MAIL 
...
if [ "$promo" == "" ] || [ "$user" == "" ] ; then
   ...
   pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de El Archivo de Regularizacion DTH no se encuentra en forma Correcta" 
   pMessage "Termino subproceso"
   pMessage "************************************" 	    
   pMessage "************************************"
   cat $FILELOG >> $FILELOGPR
   ...
   do
	...
	if [ "$FILEDATATEMP" = "" ] || [ $CANT_DATATEMP = 0 ] ; then	  
	   ...
	   pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de El Archivo de Regularizacion DTH no contiene datos validos" 
	   echo "Buen día, el archivo $NomArchivo en la carpeta $RUTANAME no contiene datos validos a procesar." $'\n' "Favor de atender este inconveniente" $'\n' "Gracias."$'\n'  | mail -s "DTH - POSTPAGO: El Archivo de Regularizacion DTH no contiene datos validos." $IT_MAIL 
	   #pMessage "Ruta del Archivo log : " $FILELOG
	   pMessage "Termino subproceso"
	   pMessage "************************************" 	    
	   pMessage "************************************" 
	...
	pMessage "Se procede a importar los datos del archivo de entrada a la tabla de Regularizacion DTH"
	...
	if [ $VALIDAT_CTL -ne 0 ] ;
		...
		echo "Buen día, ocurrio un error al momento de importar los datos del archivo $DIRFALLOSDTH/$NomArchivo a la tabla de regularizacion DTH." $'\n' "Favor de atender este inconveniente" $'\n' " Gracias." $'\n'  | mail -s "Error al importar datos" $IT_MAIL
		cat $CTL_LOG >> $FILELOG
		rm -f $CTL_LOG 
		pMessage "Termino subproceso"
		pMessage "************************************" 
...
pMessage "Se procede a ejecutar el SP PKG_CC_PTOSFIJA.ADMPSI_REGDTH_HFC"
	...
	BEGIN
		...
		begin
			...
			$PCLUB_OW.PKG_CC_PTOSFIJA.ADMPSI_REGDTH_HFC('6',v_usuario,v_fecha,'$NomArchivo',
...
EOP
	...
	if [ ${VALIDA_EJEC_SP} -ne 0 ] || [ ${VALIDA_EJEC_SP_B} -ne 0 ] ; then
		...
		pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de IMPORTACION DE PUNTOS POR REGULARIZACION DTH – Se encontraron errores" 
		echo "Buen día, ocurrio un problema al ejecutar el SP de PKG_CC_PTOSFIJA.ADMPSI_REGTH_DTH." $' \n' "Favor de atender este inconveniente" $'\n' "Gracias." $'\n'  | mail -s "IMPORTACION DE PUNTOS POR REGULARIZACION DTH – Se encontraron errores" $IT_MAIL 		
		pMessage "Se envió correo Fecha y Hora: $demora" 		
		pMessage "Termino subproceso"
	    pMessage "************************************" 	    
		pMessage "************************************" 
	...
	if [ "$FINDFILE" = "" ] && [ "$FINDBKP" = "" ] ; then
		...
		pMessage "No existe ninguno de estos archivos $ARCHNAME" 
		pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de El Archivo de Regularizacion DTH no se encuentra en la ruta" 
		echo "Buen día, no se encontraron los siguientes archivos $ARCHNAME en la carpeta de $RUTANAME." $'\n' "Favor de atender este inconveniente" $'\n' "Gracias."$'\n'  | mail -s "DTH - POSTPAGO: REGULARIZACION, no se encuentra el archivo." $IT_MAIL 
		pMessage "Se envió correo Fecha y Hora: $demora"    
		pMessage "Termino subproceso"
	    pMessage "************************************" 	    
		pMessage "************************************"
...
pMessage "Termino subproceso Asignacion de Puntos por Regularizacion DTH ClaroClub"
pMessage "********** FINALIZANDO SUBPROCESO ********** " 
pMessage "Fin de subproceso "
pMessage "************************************" 
#pMessage "Ruta del Archivo log : ${FILELOG}" 
