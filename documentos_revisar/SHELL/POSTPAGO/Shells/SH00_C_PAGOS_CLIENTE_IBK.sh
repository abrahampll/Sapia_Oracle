...
GENERAR_FILE(){
 ...
 do
		...
		MONTCLI=`echo $FIELD01 | sed 's/\\r//g'|awk 'BEGIN{FS="|"} {print $5}' `
		TIPOPER=`echo $FIELD01 | sed 's/\\r//g'|awk 'BEGIN{FS="|"} {print $4}' `
		#FECREG=`echo $FIELD01 | sed 's/\\r//g'|awk 'BEGIN{FS="|"} {print $9}' `
		NOMBREFILE=$FILE
		echo "1|5|PAGO TARJETA DE CREDITO IBK|$CODCLIENTE|$DNICLIENTE|$MONTCLI|$TIPOPER||$NOMBREFILE" >> $FILETEMP
 done < $FILEDATA
...
sh -x /home/usrclaroclub/CLAROCLUB/Interno/Postpago/Shells/SH00_C_PAGOS_CLIENTE_POSTPAGO.sh