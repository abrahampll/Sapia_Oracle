...
sqlplus -s $USUARIO/$PASSWOR@$BASEDAT << EOP >$FILEACTPRE
  ...
  begin
    ...
    $PCLUB_OW.PKG_CC_PREPAGO_RECAR.ADMPSS_CATEG_PRERECARGA_WA(k_fecha, k_coderror, k_descerror);
...
$PCLUB_OW.PKG_CC_PREPAGO_RECAR.ADMPSI_PRERECAR_WA(v_USUARIO, v_FECHA, K_NUME_PROCES, v_CODERROR, v_DESCERROR, v_NUMREGTOT, v_NUMREGPRO, v_NUMREGERR);
...
sqlplus -s $USUARIO/$PASSWOR@$BASEDAT << EOP >$FILEACTPRE
  WHENEVER SQLERROR EXIT SQL.SQLCODE;
	...
	begin
		...
		${PCLUB_OW}.PKG_CC_PREPAGO_RECAR.ADMPSI_FECHAFALTA_PRO(K_FECHA,K_CANTSIN_PROC,K_CANTSIN_CATE,K_CODERROR,K_DESCERROR);
...
${PCLUB_OW}.PKG_CC_PREPAGO_RECAR.ADMPSI_PRERECAR_CONF(K_NUME_PROCES, k_count, v_CODERROR, v_DESCERROR);
...
NROPARAMETROS=$#
	...
	if [ $NROPARAMETROS -eq 0 ]; then
		...
		pMessageIni 1 $FILELOG "Se ejecuta el SP PKG_CC_PREPAGO_RECAR.ADMPSI_FECHAFALTA_PRO del proceso"
				...
				if [ $CODIGOMSG -eq 1 ];then				
					...
					if [ $CANTVAL -eq 0 ];then #si no hay registros pendientes de procesar.
						...
						if [ $ESTADO1 -eq 1 ] ; then
							...
							pMessageIni 1 $FILELOG "Se ejecuta el SP PKG_CC_PREPAGO_RECAR.ADMPSS_CATEG_PRERECARGA del proceso $PROC"
						...
						if [ $CANTVALCAT -eq 0 ];then #si no hay registros con el campo de categoria null
							...
							pMessageIni 1 $FILELOG "Se ejecuta el SP PKG_CC_PREPAGO_RECAR.ADMPSS_CATEG_PRERECARGA_WA del proceso $PROC"
							#Inicio de Categorización
							IDEN_FILE_TMP=$(GetNumRandom )
							FILE_PREREC_TMP=$DIRTMP/FILE_PRERE_$IDEN_FILE_TMP.tmp
							RPTA_CATPRE=$( ActualizaPreRecarga $USER_BD $CLAVE_BD $SID_BD $FILE_PREREC_TMP )
		...
		if [ $RPTA_CATPRE_H -eq 1 ]; then
				...
				pMessageIni 0 $FILLOG "Se ejecuta el procedimiento de carga PKG_CC_PREPAGO_RECAR.ADMPSI_PRERECAR_WA"	
					
				IDENTEMP=$(GetNumSecuencial)
				FILETMP=$DIRTMP/"SH010_RECARGA_${IDENTI}_${IDENTEMP}.tmp"			
				EXECPREREC $PROCES $FILETMP
				pMessageIni 0 $FILLOG "Se termina de ejecutar el procedimiento de carga PKG_CC_PREPAGO_RECAR.ADMPSI_PRERECAR_WA"	
				...
				if [ $VALIDAT_CTL1 -ne 0 ]; then
					...
					pMessageIni 0 $FILLOG `date +"%Y-%m-%d %H:%M:%S"` "ERROR en la ejecucion del procedure de ASIGNACION DE PUNTOS POR	RECARGAS CLAROCLUB PREPAGO: ${PCLUB_OW}.PKG_CC_PREPAGO.ADMPSI_PRERECAR_WA. Contacte al administrador."$'\n' 
					echo  $'\n'"Error al ejecutar el procedure ASIGNACION DE PUNTOS POR RECARGAS CLAROCLUB PREPAGO: ${PCLUB_OW}.PKG_CC_PREPAGO_RECAR.ADMPSI_PRERECAR_WA"| mail -s "ASIGNACION DE PUNTOS POR RECARGAS CLAROCLUB PREPAGO – error al ejecutar el procedure" $IT_OPERADOR
