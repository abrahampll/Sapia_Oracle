...
BEGIN
	$PCLUB_OW.PKG_CC_BONOS.ADMPSS_LST_CLIENTE_BONOS(K_USUARIO,K_REGISTRO,K_CODERROR,K_DESCERROR);
  IF K_CODERROR <> 0 THEN
		DBMS_OUTPUT.PUT_LINE(TO_CHAR(K_CODERROR) ||'|' || 'ORA-9999' || '-' || K_DESCERROR);
  ELSE
...
BEGIN
	$PCLUB_OW.PKG_CC_BONOS.ADMPSU_UPD_ENTREGA_BONOS(K_NUME_PROCES,K_CANT_PROCES,K_CANT_EXITOS,K_CANT_ERRADO,K_FLAG_EXITOS,K_MENS_TRANSA);
  IF K_FLAG_EXITOS <>0 THEN
    DBMS_OUTPUT.PUT_LINE(TO_CHAR(K_FLAG_EXITOS) ||'|' || 'ORA-9999' || '-' || K_MENS_TRANSA);
  ELSE
    DBMS_OUTPUT.PUT_LINE(TO_CHAR(K_CANT_PROCES) ||'|' || TO_CHAR(K_CANT_EXITOS) ||'|' || TO_CHAR(K_CANT_ERRADO));
  END IF;
EXCEPTION
  WHEN OTHERS THEN
    DBMS_OUTPUT.PUT_LINE('ERROR: '||TO_CHAR(SQLCODE)||' MSG: '||SQLERRM);
...
if [ $NROPARAMETROS -eq 0 ]; then
  ...
  clear
	...
	if [ $STADOOBTENE -eq -1 ]; then
		...
		pMessage 1 $FILELOG "Obteniendo informacion de bonos a dar de baja"
		HORAPROCES2=`date +'%H%M%S'`
		TIEMPOPROCE=$(InterlTime $HORAPROCES1 $HORAPROCES2)
		pMessage 1 $FILELOG "SE INSERTARON : $NROLINEAS REGISTROS EN : $TIEMPOPROCE"
	fi
  ...
  ######################################
	...
	if [ $STADOUPDATE -eq 1 ]; then
		...
		MensajErr=`head -1 $REGRESUMEN`
		MensWrite=`echo $MensajErr | sed 's/\\r//g'|awk 'BEGIN{FS="|"} {print $2}'`
		pMessage 1 $FILLOG "ERROR AL PROCESAR BONOS DE FIDELIDAD PARA LOS REGISTROS DEL GRUPO NRO $PROCES : $MensWrite" 
		FinalShell 0 $FILLOG $HORCHI "$IDENTI"
		Send_Email "$IT_SOAP" "$ASUNTO_MAIL PROCESO $IDENTI" "FIN DE PROCESO $IDENTI" "$FILLOG"
		exit
	fi
