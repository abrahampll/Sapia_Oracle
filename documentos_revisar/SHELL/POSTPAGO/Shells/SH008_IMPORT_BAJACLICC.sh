...
if [ "$FILEDATA" = "" ] ; then
   ...
   pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de El Archivo de baja de clientes no se encuentra en la ruta" 
...
if [ $CANT_DATA = 0 ] ; then
	...
	pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de El Archivo de baja de clientes se encuentra vacio." 
...
if [ ${VALIDA_EJEC_SP} -ne 0 ] || [ ${VALIDA_EJEC_SP_B} -ne 0 ] ; then
    pMessage "Hora y Fecha: $demora"
    pMessage "Hubo un error durante la ejeción del SP pkg_cc_procacumula.admpsi_bajaclic" 
    pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de IMPORTACION DE Baja de clientes – Se encontraron errores" 
    echo "Buen día, ocurrio un problema al ejecutar el SP de pkg_cc_procacumula.admpsi_bajaclic." $' \n' "Favor de atender este inconveniente" $'\n' "Gracias." $'\n'  | mail -s "IMPORTACION DE Baja de clientes – Se encontraron errores" $IT_MAIL 		
    pMessage "Se envió correo Fecha y Hora: $demora"    
	exit
fi
...
if [ ${VALIDA_EJEC_SP} -ne 0 ] || [ ${VALIDA_EJEC_SP_B} -ne 0 ] ; then
    ...
    pMessage "Hora y Fecha: $demora"
    pMessage "Hubo un error durante la ejeción del SP pkg_cc_procacumula.admpsi_ebajaclic"        
	exit
fi
...
pMessage "Se finalizó el proceso de Ejecución de Baja de clientes" 
