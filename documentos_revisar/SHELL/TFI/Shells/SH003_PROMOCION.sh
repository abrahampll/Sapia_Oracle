#! /bin/ksh
#*************************************************************
#* DESCRIPCION           : Puntos por Promocion TFI
#* EJECUCION             : Control-D                            
...
CONTROL=$DIRCTL/importaPPromTFI.ctl   
BAD=$DIRFALLOS/importaPPromTFI_BAD_$FECHA.bad
FILELOG=$DIRLOG/SH003_PROMOCION_$FECHA.log
CTL_LOG=$DIRLOG/CTL003_LOG_$FECHA.log
...
if [ "$FILEDATA" = "" ] ; then
   ...
   pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de El Archivo de Promocion TFI no se encuentra en la ruta" 
...
if [ $CANT_DATA = 0 ] ; then
	...
	pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de El Archivo de Promoción TFI se encuentra vacio." 
	echo "Buen día, los siguientes archivos $ARCHNAME en la carpeta de ORIGEN no contienen datos." $'\n' "Favor de atender este inconveniente" $'\n' " Gracias." $'\n'  | mail -s "El Archivo de Promoción TFI se encuentra vacio" $IT_MAIL
...
if [ "$promo" == "" ] || [ "$user" == "" ] ; then
   ...
   pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de El Archivo de Promoción TFI no se encuentra en forma Correcta" 
   pMessage "Termino subproceso"
   pMessage "************************************" 	    
   pMessage "************************************"
   cat $FILELOG >> $FILELOGPR
   ...
   ARCHIVO=`echo $ARCHNAME | awk 'BEGIN{FS="Promocion/"} {printf $2 }'`
	...
	if [ "$FILEDATATEMP" = "" ] || [ $CANT_DATATEMP = 0 ] ; then
	   ...
	   pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de El Archivo de Promoción TFI no contiene datos válidos" 
	   echo "Buen día, el archivo $NomArchivo en la carpeta $RUTANAME no contiene datos validos a procesar." $'\n' "Favor de atender este inconveniente" $'\n' "Gracias."$'\n'  | mail -s "TFI: El Archivo de Promoción TFI no contiene datos válidos." $IT_MAIL 
	   #pMessage "Ruta del Archivo log : " $FILELOG
	   pMessage "Termino subproceso"
	   pMessage "************************************" 	    
	   pMessage "************************************" 
	...
	pMessage "Se procede a importar los datos del archivo de entrada a la tabla de Promoción TFI"
	...
	if [ $VALIDAT_CTL -ne 0 ] ;
		...
		echo "Buen día, ocurrio un error al momento de importar los datos del archivo $BAD a la tabla de Promoción TFI." $'\n' "Favor de atender este inconveniente" $'\n' " Gracias." $'\n'  | mail -s "Error al importar datos" $IT_MAIL
		pMessage "Termino subproceso"
	    pMessage "************************************" 	    
		pMessage "************************************" 
		echo $'\n'
	...
	pMessage "El proceso de importación culmino satisfactoriamente"

	rm -f $CTL_LOG 

	cp $FILEDATA $DIRPROC_PROMO
	...
	pMessage "Se procede a ejecutar el SP PKG_CC_PTOSTFI.ADMPSI_PROMOCION_TFI"
...
BEGIN
	...
	begin
		...
		$PCLUB_OW.PKG_CC_PTOSTFI.ADMPSI_PROMOCION_TFI(v_fecha,'$NomArchivo',k_coderror,k_descerror,
...
EOP
	...
	if [ ${VALIDA_EJEC_SP} -ne 0 ] || [ ${VALIDA_EJEC_SP_B} -ne 0 ] ; then
		pMessage "Hora y Fecha: $demora"
		pMessage "Hubo un error durante la ejeción del SP PKG_CC_PTOSTFI.ADMPSI_PROMOCION_TFI" 
		pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de IMPORTACION DE PUNTOS POR PROMOCION TFI – Se encontraron errores" 
		echo "Buen día, ocurrio un problema al ejecutar el SP de PKG_CC_PTOSTFI.ADMPSI_PROMOCION_TFI." $' \n' "Favor de atender este inconveniente" $'\n' "Gracias." $'\n'  | mail -s "IMPORTACION DE PUNTOS POR PROMOCION TFI – Se encontraron errores" $IT_MAIL 		
		pMessage "Se envió correo Fecha y Hora: $demora" 		
		pMessage "Termino subproceso"
	    pMessage "************************************" 	    
		pMessage "************************************" 
	...
	FINDBKP=`find $DIRPROC_PROMO/$NomArchivo`
	
	if [ "$FINDFILE" = "" ] && [ "$FINDBKP" = "" ] ; then
		pMessage "Hora y Fecha: $demora"
		pMessage "No existe ninguno de estos archivos $ARCHNAME" 
		pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de El Archivo de Promoción no se encuentra en la ruta" 
		echo "Buen día, no se encontraron los siguientes archivos $ARCHNAME en la carpeta de $RUTANAME." $'\n' "Favor de atender este inconveniente" $'\n' "Gracias."$'\n'  | mail -s "TFI: PROMOCION, no se encuentra el archivo." $IT_MAIL 
		pMessage "Se envió correo Fecha y Hora: $demora"		
		pMessage "Termino subproceso"
	    pMessage "************************************" 	    
		pMessage "************************************"
...
pMessage "Termino subproceso Asignacion de Puntos por Promoción TFI ClaroClub"
pMessage "********** FINALIZANDO SUBPROCESO ********** " 
pMessage "Fin de subproceso "
pMessage "************************************" 
#pMessage "Ruta del Archivo log : ${FILELOG}" 
