...
FARCH=`date +%Y%m%d`
FILELOG=$DIRLOGHFC/SH002_PROC_VTOPUNTO_$FECHA.log
ARCHSHELL=SH002_PROC_VTOPUNTO.sh
...
echo "Ruta Shell: $DIR_HFC_SHELL/$ARCHSHELL"

	
pMessage "Se procede a ejecutar el SP PKG_CC_PTOSFIJA.ADMPSI_PREVENCPTO"
...
if [ ${VALIDA_EJEC_SP} -ne 0 ] || [ ${VALIDA_EJEC_SP_B} -ne 0 ] ; then
    ...
    pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de VENCIMIENTO DE PUNTOS HFC – Se encontraron errores" 
    echo "Buen día, ocurrio un problema al ejecutar el SP de PKG_CC_PTOSFIJA.ADMPSI_PREVENCPTO." $' \n' "Favor de atender este inconveniente" $'\n' "Gracias." $'\n'  | mail -s "VENCIMIENTO DE PUNTOS HFC – Se encontraron errores" $IT_MAIL 		
