#*************************************************************
#Programa      :  SH007_IMPORT_RENCONTCC_SMS.sh	
...
pMessage () {   
   ...
   echo "($LOGDATE) $*" 
   echo "($LOGDATE) $*"  >> $DIRLOGPOST/$LOGFILE
}
NPROCESO=$$
FECHA=`date +%d%m%Y`
...
USER_SERV=`whoami`
SHELL=SH007_IMPORT_RENCONTCC_SMS.sh	
LOGFILE=LOG_IMP_CONT_${FECHA_HORA}.log
ARCH_CUR=CURSOR_IMP_CONT_$FECHA.tmp 
ARCH_SMS=$DIRPROCPOST/$ARCH_CUR
VAR_COUNT=0
contador=1
...
$PCLUB_OW.PKG_CC_ENVIO_SMS.ADMPSS_COUNT_SMS('RENOCONT','USRCONT',V_COUNT, v_CODERROR, v_DESCERROR);
...
ARCH_TP=TR_SH007_$FECHATRAMA.txt
ARCH_WS=Respuesta_CONT_$FECHATRAMA.ws
TMP=$DIRPROCPOST/$ARCH_TP
RESWS=$DIRPROCPOST/$ARCH_WS
...
$PCLUB_OW.PKG_CC_ENVIO_SMS.ADMPSI_ENVIOSMS_PROCESOS('RENOCONT','USRCONT',v_CODERROR, v_DESCERROR, CUR_SMS);
