#!/bin/sh -x
#*************************************************************
#Programa         : SH005_AFILIACION_REC.sh
#Descripción      : Registro Automático por Recargas
#Fecha Creación   : 08/08/2013
#Usuario Creación : Roxana Chero
#*************************************************************
...
USER_PROC="USRAFILAUTOM"
PREF_SHELL="SH005_"
NOMB_SHELL="AFILIACION_REC"
DESC_PROCESO="Registro Automático por Recargas"
ASUN_MAIL="ERROR: $DESC_PROCESO"
SLDO_MAIL="\n\nPor favor atender este inconveniente. \nGracias"
TIPO_BONO_FIDEL="M"
NOMB_ARCH="AFIL_RECARGA"
EXTE_ARCH=".CCL"
PRMT_ARCH=$1
#VARIABLES ARCHIVOS
DIRD_ARCHDOCU=$DIRDOCUMENTOSALTA
DIRD_ARCHPROC=$DIRPROCESADOALTA
FILE_CTL=$DIRCTL/CTL005_${NOMB_ARCH}.ctl
...
CTRL_LOG=$DIRLOG/CTL005_${NOMB_SHELL}_$FECHA_HORA.log
EMAIL=$IT_OPERADOR
pMessage () {
...
InicioShell(){
pMessage "-------------------------------------------------------"
pMessage "|      INICIANDO REGISTRO AUTOMÁTICO POR RECARGAS     |"
pMessage "-------------------------------------------------------"
...
BEGIN
$PCLUB_OW.PKG_CC_BONOS.ADMPSS_TMP_AFILXREC('$NOMBARCH',V_NUMREG,V_CODERROR,V_DESCERROR);
...
V_DESCERROR VARCHAR2(250);
BEGIN
$PCLUB_OW.PKG_CC_BONOS.ADMPSI_AFILXREC('$NOMBARCH','$USER_PROC',V_NUMREGTOT,V_NUMREGVAL,V_NUMREGERR,V_CODERROR,V_DESCERROR);
...
V_DESCERROR VARCHAR2(250); 
BEGIN
$PCLUB_OW.PKG_CC_BONOS.ADMPSS_TMP_EAFILXREC('$NOMBARCH',V_CUR_LISTA,V_CODERROR,V_DESCERROR);
...
if [ $ESTADO -ne 1 ] ; then
	pMessage "Hora y Fecha: `date +'%d-%m-%Y %H:%M:%S'`"
	pMessage "ERROR: Ocurrió un error en la ejecución del procedure de $DESC_PROCESO: ${PCLUB_OW}.PKG_CC_BONOS.ADMPSS_TMP_AFILXREC. Contacte al administrador."
	pMessage "A continuación se enviará un correo a $EMAIL con el asunto: $ASUN_MAIL" 
	pMessage "Verifique el log para mayor detalle $FILE_LOG"
	pMessage "Terminó subproceso"
	pMessage "************************************" 
    echo -e "Error al ejecutar el procedure de $DESC_PROCESO: ${PCLUB_OW}.PKG_CC_BONOS.ADMPSS_TMP_AFILXREC.${SLDO_MAIL}" | mail -s "$ASUN_MAIL" $EMAIL
	echo $'\n'
...
ESTADO=$(ProcesarBono ${FILE_NAME} ${FILE_PROCESO})
if [ $ESTADO -ne 1 ] ; then
	pMessage "Hora y Fecha: `date +'%d-%m-%Y %H:%M:%S'`"
	pMessage "ERROR: Ocurrió un error en la ejecución del procedure de $DESC_PROCESO: ${PCLUB_OW}.PKG_CC_BONOS.ADMPSI_AFILXREC. Contacte al administrador."
	pMessage "A continuación se enviará un correo a $EMAIL con el asunto: $ASUN_MAIL"
	pMessage "Verifique el log para mayor detalle $FILE_LOG"
	pMessage "Terminó subproceso"
	pMessage "************************************" 
    echo -e "Error al ejecutar el procedure de $DESC_PROCESO: ${PCLUB_OW}.PKG_CC_BONOS.ADMPSI_AFILXREC.${SLDO_MAIL}" | mail -s "$ASUN_MAIL" $EMAIL
	echo $'\n'
...
if [ $CANTREGERR -gt 0 ] ; then
	...
	if [ $ESTADO -ne 1 ] ; then
		pMessage "Hora y Fecha: `date +'%d-%m-%Y %H:%M:%S'`"
		pMessage "ERROR: Ocurrió un error en la ejecución del procedure de $DESC_PROCESO: ${PCLUB_OW}.PKG_CC_BONOS.ADMPSS_TMP_EAFILXREC. Contacte al administrador."
		pMessage "A continuación se enviará un correo a $EMAIL con el asunto: $ASUN_MAIL"
		pMessage "Verifique el log para mayor detalle $FILE_LOG"
		pMessage "Terminó subproceso"
		pMessage "************************************" 
		echo -e "Error al ejecutar el procedure de $DESC_PROCESO: ${PCLUB_OW}.PKG_CC_BONOS.ADMPSS_TMP_EAFILXREC.${SLDO_MAIL}" | mail -s "$ASUN_MAIL" $EMAIL
		rm -f $FILE_ERR
	else 
		pMessage "Hora y Fecha: `date +'%d-%m-%Y %H:%M:%S'`"
