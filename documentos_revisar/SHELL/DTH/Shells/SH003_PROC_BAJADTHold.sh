...
if [ $VALIDAT_CTL -ne 0 ]
	...
	#echo "Buen día, ocurrio un error al momento de importar los datos a la tabla de Baja. Archivo BAD: $BAD" $'\n' "Favor de atender este inconveniente" $'\n' " Gracias." $'\n'  | mail -s "DTH-POSTPAGO: Error BSCS." $IT_MAIL
    BAD_ZIP=bajaClienteDTH_BAD_$FECHA.zip
	BAD_NOM=bajaClienteDTH_BAD_$FECHA.bad
	cd $DIRFALLOSDTH
	zip $BAD_ZIP $BAD_NOM
	(echo "Buen día, ocurrio un error al momento de importar los datos a la tabla de Baja. Se adjunta el archivo BAD." $'\n' "Favor de atender este inconveniente" $'\n' " Gracias." $'\n' ; uuencode $BAD_ZIP $BAD_ZIP) | mail -s "DTH-POSTPAGO: Error DATA BSCS." $IT_MAIL
    rm -f $BAD_ZIP
	cd $DIR_DTH_SHELL
	#pMessage "Termino proceso"
    pMessage "************************************" 
...
if [ ${VALIDA_EJEC_SP} -ne 0 ] || [ ${VALIDA_EJEC_SP_B} -ne 0 ] ; then
    pMessage "Hubo un error durante la ejeción del SP PKG_CC_PTOSFIJA.ADMPSI_BAJACLIENTEDTH" 
...
if [ ${VALIDA_EJEC_SP} -ne 0 ] || [ ${VALIDA_EJEC_SP_B} -ne 0 ] ; then
    ...
    pMessage "Hubo un error durante la ejecución del SP PKG_CC_PTOSFIJA.ADMPSI_EBAJACLIENTEDTH"  
	echo "Buen día, Hubo un error durante la ejecución del SP PKG_CC_PTOSFIJA.ADMPSI_EBAJACLIENTEDTH." $'\n' "Favor de atender este inconveniente" $'\n' "Gracias."$'\n'  | mail -s "DTH-POSTPAGO: Baja de clientes." $IT_MAIL 	
	exit
else
	if [ $CANT_DATA != 0 ] ; then
