...
if [ "$FILEDATA" = "" ] ; then
   ...
   pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de El Archivo de Baja no se encuentra en la ruta" 
...
if [ $CANT_DATA = 0 ] ; then
	...
	pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de El Archivo de Baja se encuentra vacio." 
...
if [ $VALIDAT_CTL -ne 0 ]
	...
	#echo "Buen día, ocurrio un inconveniente al momento de importar los datos a la tabla de Baja. Archivo BAD: $BAD" $'\n' "Favor de atender este inconveniente" $'\n' " Gracias." $'\n'  | mail -s "DTH-POSTPAGO: BSCS." $IT_MAIL
    BAD_ZIP=bajaClienteDTH_BAD_$FECHA.zip
	BAD_NOM=bajaClienteDTH_BAD_$FECHA.bad
	cd $DIRFALLOSDTH
	zip $BAD_ZIP $BAD_NOM
	(echo "Buen día, ocurrio un inconveniente al momento de importar los datos a la tabla de Baja. Se adjunta el archivo BAD." $'\n' "Favor de atender este inconveniente" $'\n' " Gracias." $'\n' ; uuencode $BAD_ZIP $BAD_ZIP) | mail -s "DTH-POSTPAGO: DATA BSCS." $IT_MAIL
    rm -f $BAD_ZIP
	cd $DIR_DTH_SHELL
	#pMessage "Termino proceso"
    pMessage "************************************" 
...
BEGIN
	...
	dbms_output.put_line('Codigo: ' || k_coderror || '|Mensaje: ' || k_descerror || '|Nº total de registros: ' || k_numregtot || '|Nº de registros procesados: ' || k_numregpro || '|Nº de registros con errores: ' || k_numregerr);
...
if [ ${VALIDA_EJEC_SP} -ne 0 ] || [ ${VALIDA_EJEC_SP_B} -ne 0 ] ; then
    pMessage "Hubo un durante la ejeción del SP PKG_CC_PTOSFIJA.ADMPSI_BAJACLIENTEDTH" 
    pMessage "A continuación se enviará un correo a $IT_MAIL con el asunto de BAJA DE CLIENTES – Se encontraron errores" 
    echo "Buen día, ocurrio un problema al ejecutar el SP de PKG_CC_PTOSFIJA.ADMPSI_BAJACLIENTEDTH." $' \n' "Favor de atender este inconveniente" $'\n' "Gracias." $'\n'  | mail -s "DTH-POSTPAGO: BAJA DE CLIENTES – Se encontraron errores" $IT_MAIL 		
    pMessage "Se envió correo Fecha y Hora: $demora"    
	exit
fi
...
if [ ${VALIDA_EJEC_SP} -ne 0 ] || [ ${VALIDA_EJEC_SP_B} -ne 0 ] ; then
    ...
    pMessage "Hubo un  durante la ejecución del SP PKG_CC_PTOSFIJA.ADMPSI_EBAJACLIENTEDTH"  
	echo "Buen día, Hubo un  durante la ejecución del SP PKG_CC_PTOSFIJA.ADMPSI_EBAJACLIENTEDTH." $'\n' "Favor de atender este inconveniente" $'\n' "Gracias."$'\n'  | mail -s "DTH-POSTPAGO: Baja de clientes." $IT_MAIL 	
	exit
else
	if [ $CANT_DATA != 0 ] ; then
